...
### FR-11.1c 上游检索：宽召回 + 模型语义重排 + 读正文 — 块1–3 已实现，待 Windows 验（2026-08-10）
...
- **待 Windows 验**：①搜索整体耗时可接受（多一次模型调用 + K 次并发抓取）②中文查询的摘录不乱码
  ③`read_top_n: 0` / `model_rerank: false` 能干净回到老行为 ④受阻页的提示能引导模型改用 web_fetch。
