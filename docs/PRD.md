...
- FR-11.1 **联网检索**：`web_search` + `web_fetch` 只读工具（查文档/报错/库用法——当前功能面最大缺口）。
  - FR-11.1d **托管检索源（Firecrawl）**：只替换最脆的召回那一跳，工具面零变化；没 key 自动降级回免 key 链路。
...
### FR-11.1d 托管检索源（Firecrawl）：只补最脆的那一跳 — ✅ 已定版 v3.71.0（2026-08-20，Windows 真机验证通过）；**配置入口 ✅ 已定版 v3.74.0**（2026-08-21，Windows 真机验证通过）

**要解决的**：FR-11.1c 把上游修到了"候选池 + 排序 + 读正文"，但**召回本身**还是免 key 抓页面再解析
（Bing RSS / DDG lite）——这是整条栈里最脆的一环，随时可能被改版、反爬、空壳打穿
（v3.43「浏览器搜索返回空壳」就是同一类事故）。而它后面的 RRF 融合、语义重排、控源多样、
读正文、质量闸（块H1/H2）、换源阶梯**全都与召回源解耦**。

**做法：把 Firecrawl 做成 `_search_one` 的一个引擎，不是一个新工具。**

- **工具面零变化**：模型仍然只看到 `web_search`，不必在两个搜索之间选路。
  理由是既有教训：强模型在能凑合时会绕开新工具（trace_run / search_code 都中招），
  且 v3.53 已立"由代码自动切换、不让模型选路"。
- **四档 `web.firecrawl`**：`off` / `fallback` / `primary`（默认，v3.72.0 起）/ `always`。
  **没有 `FIRECRAWL_API_KEY` 一律等于 off**——开箱不预设 provider（v3.56 的立场），
  且**免 key 链路必须永远可用**：不带凭据也能搜是 hermes 的底线能力。
  配置写脏值（拼错/写 true）一律归 off，绝不因手滑变成"每次都计费"。
- **fallback 的升级闸**（三条判据，全部可证伪）：①零结果；②召回不足要的一半；
  ③**复用块H1 `ResearchEvaluator` 已有的 blocker**（如"有标价却无一在预算内"），不另造判据。
  命中则**代码自己**换源补搜、再一起进 RRF；head 里打 `[已换源] 原因` ——动用付费源要说出来。
  刻意不看"对不对题"那类语义判断：那是块H3a 模型裁判的活，放这里又慢又贵。
- **默认档为什么是 primary（而不是 fallback，也不是 always）**：两步都由实测定的。
  - **fallback → primary**（2026-08-20 真机）：用户反馈"全程没触发过"——升级闸的三条判据
    在真实使用里几乎不成立，省下的配额纯属闲置。既然给了更好的源，就该默认用它。
  - **不上 always**（2026-08-21 A/B，6 个真网任务各 1 轮，credits 用 `team/credit-usage`
    前后差值实测）：通过率 6/6 vs 6/6、credits 39 vs 37（都在噪声内），
    但**耗时 570s → 868s，且 6 个任务无一例外全都变慢**（+52%）。
    always 花掉的不是配额、是**时间**：每次读页多绕一趟 Firecrawl 渲染，
    而这些页免 key 链路本来就读得动。局限：n=1，真网每跑一变；但方向一致。
  - 配额账仍然成立（批量评测里尤其）：`search(limit=5)` = **2 credits**、免费档 **1000/月**，
    一个研究型任务搜 **7–8 次** → 一个月只够 60–70 个任务。
  **另有一条反直觉的**：全量并进 RRF 未必更好——RRF 让"多引擎都有"的上浮，
  Firecrawl 独有的好结果只有一票，可能反被压下去。**这条已经用评测测过了**（见上）。

**降级纪律（自检钉死）**：没 key / key 失效 / Firecrawl 抽风 / 配额用尽——
都只能让搜索"少一个源"，绝不能让搜索失败；连免 key 链路也挂了才报错，且错误里**两边原因都带上**。

**自测**：`tests/test_firecrawl.py` 12 测（不联网、不用 key，且临时清掉环境里的真 key，
绝不依赖本机 `.env`）。真网端到端实测三场：普通事实查询**不升级、0 credits**；
带预算约束的查询按块H1 的保守判据**也不升级**（搜到的是讨论页、无标价，只记 signal 不判 blocker）；
模拟免 key 链路被反爬拦死 → **Firecrawl 救场，5 条结果、2 credits**，head 正确标注换源原因；
再模拟 Firecrawl 也挂 → 可读 `ToolError`，带上两边原因。

**密钥**：只在 `.env`（已 gitignore），绝不进代码/配置/文档。
**配置入口（设置面板「🔍 联网检索」页）**：key 与档位在同一屏，填完写回 `<APP_DIR>/.env`、即时生效。

> 为什么专门开一页：2026-08-21 的一次真实排查暴露了产品缺口——key 只从环境读、档位在
> `config.yaml`，**两个条件都成立才生效**，而任一边缺了都只表现为"搜索照旧走 bing"，
> 且 `.env.example` 与打包模板里**根本没提过这个环境变量名**。能填只是及格线，
> 这一页真正的职责是**说破实际生效档位**：没 key / 配额用尽时，面板与左栏徽标
> 都直接报「缺 key」「配额用尽」并写明"现在走的是免 key 的 Bing + DDG"——
> 同「动用付费源要说出来」那条纪律的反面：**没用上付费源，同样要说出来**。
> 换 key 时清掉配额用尽的粘滞标记（那正是会来这一页的主要原因之一）。
> 落地机制是通用的 `CHOICES_SPEC` / `choices.json`（枚举型设置，与 `LIMITS_SPEC` 同构），
> 不是给 Firecrawl 单开的一次性旋钮。自测见 `tests/test_choices.py`（15 测）与
> `tests/web/web_search_badge.test.js`（6 测）。

**段 2（读页兜底）**：`_read_one` 抓不动、`web_fetch` 命中 `looks_blocked` 或 HTTP 直读就失败时，
在**浏览器穿透之前**插一层 Firecrawl scrape（无本地依赖，比 npx + Chrome 轻得多）。
降级阶梯定为 **HTTP → Firecrawl → 浏览器**：越往后越重、越有侵入性（浏览器带登录态，
读到的可能是私域内容；托管源只读得到公开内容，副作用更小）。

**2026-08-20 真网实测定的口径**（每条判断都对着这三个事实写）：

| 事实 | 由此定下的做法 |
|---|---|
| `scrape` = **1 credit**（`search` 是 2），1.9~6.7s | 读页兜底比搜索换源便宜，但仍要封顶 |
| 适用面是 **JS 空壳**：`app.slack.com/help` 直读 141 字符 → 渲染回 **1874 字符** | 这是它真正买得到增量的一档 |
| **打不穿强反爬/登录墙**：知乎 403 页拿回的是「你似乎来到了没有知识存在的荒原」拦截插页（242 字符），加 `proxy=stealth` 也一样（382 字符） | ①必须对**付费源产出**再判一次（`firecrawl_gain`）；②命中登录墙**直接跳过 Firecrawl**——那类页在它那儿是稳定的必然失败，试一次＝白烧 1 credit，只有带登录态的浏览器有戏 |

- **`firecrawl_gain(baseline, fetched)`（纯函数）**：空 / 仍是空壳或反爬 / 跳转插页 /
  **没比直读长出 1.5 倍** → 一律不采纳，继续降级。判据全朝"宁可继续降级"偏：
  降到浏览器只是慢几秒，把拦截页当正文交出去则是**既花了钱又给了错的东西**
  （同 ADR 0025「一个自信的错数比缺失更危险」）。跳转插页的模式**只在这一关用**，
  不进全链路共用的 `_BLOCK_MARKERS`——在那里误判会让 HTTP 直读也无谓升级。
- **每次搜索的兜底预算封到 2 次**（`FIRECRAWL_READ_BUDGET`，并发共享的信号量）：
  `read_top_n=3` 不封顶则最坏 3 credits/次搜索、叠加搜索本身的 2 credits＝5，
  封顶后最坏情况可预期。
- **共用 `web.firecrawl` 一个开关**（不长第二个旋钮，有自检钉住 registry 两处透传）。
  读页这边 **fallback 与 always 行为相同**——都只在"HTTP 不行"时才用：读页没有
  "并进 RRF"那种融合收益，同一个 URL 读两遍只会得到同一份正文，主动多打一次付费 API
  什么也买不到。`always` 只改搜索那一跳。
- **顺手补掉一张空头支票**：`web_fetch` 此前在 HTTP 层就失败（403/429）时**直接抛错**，
  浏览器兜底根本够不着——而 `web_search` 读不动时给模型的指路正是
  "用 web_fetch 读它（会自动改用浏览器）"。现在这条路也按同一条阶梯降级，
  三层全失败时**错误里带上每一层的原因**。

**段 2 自测**：`tests/test_firecrawl.py` 12 → **28 测**（仍不联网、不用 key）。
真网端到端两场实测：JS 空壳页**换源成功、浏览器一次没被叫到、消耗 1 credit**；
知乎 403 页 → Firecrawl 被 `firecrawl_gain` 判为跳转插页拒收 → 报错文案带全三层原因
（`HTTP 403；Firecrawl：跳转/拦截插页；浏览器：未接穿透`）。

### 默认档改为 `primary`（2026-08-20，用户真机反馈后拍板）

**真机反馈**：装上 key 后**全程没有触发过 Firecrawl**。也就是说 `fallback` 那三条判据
（零结果 / 召回不足一半 / 块H1 blocker）在真实使用里几乎从不成立——**那把 key 什么也没买到**。

用户的判断：既然给了更好的数据源和 key，就该**默认用它**；配额用完再退回免 key 链路。
这条推翻了段 1 「默认 fallback 是算术不是偏好」那个结论——**算术没错，但前提错了**：
省下来的配额只有在"免 key 链路真的会不够用"时才有价值，而实测它几乎总被判为够用，
于是省下的配额既没花掉、也没换来更好的结果，纯粹是闲置。

- 新增第三档 **`primary`（默认）**：Firecrawl 主搜；它没结果 / 给不够 n 条 / 不可用时，
  免 key 链路兜底（**不带凭据也能搜仍是底线能力**）。主源给了但不够时，
  免 key 链路补齐后一起进 RRF——补这一趟不额外花钱。
- **配额用尽（HTTP 402）自动退回，并且粘住**：本进程内不再打 Firecrawl。
  否则每次搜索都要先撞一次 402 才退回，白等一个往返、日志还刷屏。
  402 本身不计费，故重启后重试一次的代价可忽略——**只记在内存、不落盘**
  （落盘要处理计费周期、换 key、多进程，不值当）。
- **429 不算配额用尽**：它是限流（瞬时，退避后能恢复），当成配额用尽会**永久关掉**
  整条付费链路——误报比漏报贵，同块V 决策 6 的立场。
- **退回要说出来**：结果头部标 `[已退回] Firecrawl 配额用尽 → 本次改用免 key 链路`。
  不说的话，"结果怎么变差了"会被归到模型头上，查不到真原因。

**读页那一跳不跟着改**：仍然只在 HTTP 直读失败/读到空壳时才用 Firecrawl。
读页没有"并进 RRF"那种融合收益——同一个 URL 读两遍只会得到同一份正文，
主动多打一次付费 API 什么也买不到。

**A/B 的对象随之变了**：不再是 `fallback` vs `always`，而是 **`primary` vs `always`**
（主搜 vs 三路融合）——段 1 那条"RRF 会让独家好结果只有一票、未必占优"的怀疑，
现在正好由这两档来证伪。
...
### FR-12.3 委派并行的成本账 + 流式渲染性能 — ⚠️ 提示词部分已回退（2026-08-26）

> **先看这条**：本节里「拆分判据」相关的提示词改动**已整块回退**——真机使用后判定质量下降
> （给模型加了需要它**自我估算**的门槛「少于 5 次检索别拆」，结果把本该并行的调研当小活直接干了）。
> 检索缓存与子任务墙钟两个开关**默认关**。**三轮真跑的数据仍然有效，但别再据此改提示词。**

> **编号说明**：本节与 `modeldiag.py` / 诊断脚本注释里的 "FR-12.2 / FR-12.3" 指的是这里，
> **与本文件上方那条已撤销的「FR-12.2 诊断升级（linter/LSP）」不是一回事**——那条 2026-06-12
> 评估后不做，编号被后来的代码注释复用了。别照着旧条目找。

**起因**：用户反馈"多个调研并行跑，整体反而比主模型自己搜慢"。写 `scripts/diag_delegate_speed_realrun.py`
在开发机真跑 A/B（同题、同模型档、在 provider 层拦时间线），把"感觉慢"变成数字。

**真跑结论（三轮，2026-08-26）**：并行**确实生效**（三个 delegate 与三个 `subagent_start`
落在同一时刻，真实并发度 2.3~2.6、峰值 3，限流退避 0 次），但模型调用次数是自己干的
**3.3~4.5 倍**，吃掉并行收益还倒亏。

| 轮次 | 题 / 上限 | 单次检索 | A 段 | B 段 | 结论 |
|---|---|---|---|---|---|
| 1 | 70B 装机 / 600s | ~30-60s | 600.1s 截断 | 646.3s 截断 | 两段都没跑完，**不可比** |
| 2 | 70B 装机 / 1200s | ~90s | 1200.1s 截断 | 615.9s 跑完 | 委派赢 ≥1.95× |
| 3 | uv vs pip / 1800s | ~20-40s | **99.2s 跑完** | **169.7s 跑完** | 委派亏 **1.71×** |

**判据**：赚不赚取决于**自己做要检索多少次、每次多慢**——两三次检索能答完的题拆了必亏；
检索十几次且每次几十秒时串行会被拖垮，拆才赚。膨胀本身是**结构性**的（N 个完整 agent 循环
+ 汇总那一轮），改不掉。

> **自我修正（写下来防止再传播）**：第一轮据"子工具调用 41 次 vs 主 19 次"**推断**膨胀来自
> "子 Agent 重复搜同一片领域"，并据此做了检索缓存（下面第 1 条）。第三轮给诊断脚本加了
> **查询词打印与近似重复分析**后，证据**不支持**那个推断：跨子 Agent 逐字重复 0 次，
> 近似重复只有 1 对（0.45，同一个子 Agent 自己跟自己），三个 researcher 分工其实很干净。
> **教训**：从"次数比"推因果是不够的，判据本身也要打点。

**做了什么**（按性价比，前四条针对膨胀）：
- [x] **同回合共享检索缓存** `webcache.py`（`web.turn_cache`，**默认关**，见本节开头的回退说明）：主/子 Agent 共用一个实例、
  按回合清空、同 key 单飞、命中时明确告知模型换角度。
  **定位已降级为"顺手的保险"**——两轮真跑命中 0 次（见上方自我修正）；不命中即 no-op。
  **不要为提高命中率做模糊匹配**：代价是静默给错答案。
- [x] **researcher 收敛判据**：2~3 个独立且互相印证的来源即停；连续两次无新事实即停。治②。
  （`agent.subagent_max_steps` 本来就有，默认 15，不是缺口。）
- [x] **子任务墙钟上限** `agent.subagent_timeout_s`（**默认 0=不限**，见本节开头的回退说明）：撞上限**不杀线程**，
  走与撞步数上限相同的收尾路径并标注「未完成 · 撞时间上限」。治③。
- [~] ~~**拆分判据**写进 `delegate` 说明与 config.yaml 系统提示词~~ **已回退**：
  「先估自己做要几次检索、少于 5 次别拆」需要模型自我估算，而它估不准——
  真机当天就压垮了调研类的默认并行行为。**数据支持的是"拆得不好会亏"，不是"这类任务别拆"。**
- [x] **流式渲染按时间片不按 token**：前端 `createStreamPacer`（80ms，流式不高亮、定稿才完整渲染）
  + 后端 `ChunkCoalescer`（按对话攒 80ms/8KB，顺序是硬约束）。实测 875 字回复的
  `evaluate_js` 505 → 66 次。
- [x] **`/诊断`** 与 `modeldiag.py`：把各档实际用的模型/端点/key 来源摊开（**不打印 key**）。
- [x] **模型 API 的账户级错误**（401/402/403）与瞬时错误分开：永不重试、指名模型档与端点、
  说清与 Firecrawl 配额无关；子任务死在模型侧不再被当成"做完了"。

**不做**：跨回合缓存（陈旧结果比多搜一次危险）、查询模糊匹配（会把答非所问的结果当命中）、
跨子任务的"发现共享"（更大的一步，另议）。膨胀的④⑤是委派语义自带成本，只能靠拆分判据规避。

**验收**：✅ 第三轮两段都跑完（小题 + 1800s 上限），A 99.2s / B 169.7s。
**再跑要注意**：cap 是止损上限、不该是常态出口——墙钟贴着 cap 就说明那段被截断了，
脚本现在会自己把这句警告打出来。

---

### FR-12.4 存档可靠性：中途退出不丢已经做过的事 — ✅ 已验证通过（2026-08-27）

**起因**：用户三次报同一件事——"中途退出了，已经搜索的内容就都会丢失"。前两次都修在了别处
（换手落盘、会话提示条），第三次才找对地方：**落库时机**。

**根因**：`_run_turn` 里 `loop.run(...)` 跑完**一整轮**（最多 200 步、可能十几分钟）才走到
`history.extend()` + `_persist()`。这一轮的每一次检索，在回合结束前**一个字都没进库**。
按停止不受影响（loop 正常返回、照常落库），丢的只有崩溃/强关/断电这类**非正常退出**。
同一个根因还让换手落盘形同虚设：`_snapshot_handoff` 读 `self.history`，而本轮的工具调用
要等回合结束才进去——它记下来的是**上一轮**的现场。

**对照主流实现**（看的是本机真实存档，不是文档）：Claude Code 的 `~/.claude/projects/*.jsonl`
与 Codex 的 `rollout-*.jsonl` 都是 append-only、**一条一行、发生即写**——实测一对
tool_use→tool_result 相隔 1.2s / 84ms **各写各的行**，没有"回合末统一落盘"这一说。
两家也都**没有**"继续上次的会话"提示条：恢复是用户显式发起的（`--resume` / `codex resume`），
力气花在**让存档本身完整**。

**做了什么**：
- [x] **边跑边落库**：`AgentLoop.run(on_message=)` + `_LiveList`（append 时回调的 list 子类）。
  `run` 里有十来处 append，逐点埋钩子迟早漏一处，而漏掉那处就是"某类消息永远不落库"的静默坑。
- [x] **断尾修复** `context.repair_interrupted_tail`：断在"assistant 发了 tool_use、tool_result
  还没落库"时，历史喂回模型会被 API 打回（tool_use 无配对 tool_result），"丢内容"就变成
  "这会话再也发不出消息"。补一条注明中断的 tool_result，**不删那条 assistant**——它是
  "当时打算干什么"的唯一线索。与 Claude Code 的 `[Request interrupted by user for tool use]` 同法。
- [x] **子 Agent 的检索也落库**：`messages.sidechain` 列（对齐 Claude Code 存档的 `isSidechain`）。
  `get_messages()` **默认不返回**旁链——混进主历史就等于把子 Agent 的中间过程塞回主模型上下文，
  委派"独立上下文、只回摘要"的意义当场没了。落库只为两件事：崩溃后还在、`recall_history` 搜得到。
  `truncate_messages_after` 相应改为按**主链**计数并带走被丢弃那几轮的旁链。
- [x] **移除「继续上次的会话」提示条**：只记得住一个任务，而会话本来就带任务清单和笔记，
  再提醒一次没有增量。恢复的价值在内容不丢，不在指路。

**不做**：uuid/parentUuid 链表式历史（Claude Code 那套支持分支回溯，hermes 现有的
"覆盖式截断 + 检查点"已覆盖同类需求，改结构代价不成比例）。

**验收**：`tests/test_durable_turn.py` 10 条，含端到端——真跑 AgentLoop、第二次模型调用抛异常
模拟强杀，断言库里捞得回已搜到的内容；活性验过（摘掉修复当场红）。
**Windows 真机已验证通过**（2026-08-27）。
