...
- FR-6.4b **agent 型 MCP server（Codex 委派）**。✅ Windows 真机验证通过、已定版 v3.72.0（2026-08-21）
...
- **三档 `web.firecrawl`**：`off` / `fallback`（默认）/ `always`。
...
- **为什么默认 fallback 而不是 always**：算术不是偏好。实测 `search(limit=5)` = **2 credits**，
  免费档 **1000 credits/月**；而真跑里一个研究型任务要搜 **7–8 次**（评测实测），
  全量走 ≈ 14–16 credits/任务 → 一个月只够 60–70 个任务，用光就退回原链路、等于回到起点。
  把配额花在免 key 链路真的不行的时候，才买得到增量。
  **另有一条反直觉的**：全量并进 RRF 未必更好——RRF 让"多引擎都有"的上浮，
  Firecrawl 独有的好结果只有一票，可能反被压下去。哪个默认更优该用评测测，不该拍脑袋。
