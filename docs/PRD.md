...
### FR-14 工具产物化与句柄（大输出可寻址）— 块1–4 全部实现，待 Windows 验（2026-08-10）
...
- **待 Windows 验**：①产物路径与编码（中文环境 GBK 老坑）②后台进程 tee ③`.hermes` 不出现在改动面板/git_status
  ④大输出命令的工具结果确实变短、尾部结论还在 ⑤芯片点开能在右侧预览看到全量输出（WebView2 下的版式）。
