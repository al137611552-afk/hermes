...
前端 151/151，新增 10 条用例），Windows 真机验证通过。

### 现象与根因

用户第三次报同一件事：**中途退出，已经搜索的内容就都丢了**。前两次都往别处修了
（换手落盘、会话提示条），这次找对了地方——**落库时机**：

`_run_turn` 里 `loop.run(...)` 跑完**一整轮**（最多 200 步、可能十几分钟）才走到
`self.history.extend(new_msgs)` + `_persist(m)`。也就是说这一轮的每一次 web_search、
每一段 web_fetch 正文，在回合结束之前**一个字都没进库**。按停止不算——那条路 loop 正常
返回、照常落库；丢的只有崩溃/强关/断电这类**非正常退出**。

同一个根因还让 8-26 加的换手落盘形同虚设：`_snapshot_handoff` 读的是 `self.history`，
而本轮的工具调用要等回合结束才进去——它记下来的其实是**上一轮**的现场。

### 参考主流实现（用户提议，看的是本机真实存档）

- **Claude Code**（`~/.claude/projects/-root/*.jsonl`）：append-only JSONL，**一条消息一行**，
  每行带 `timestamp`、`uuid`/`parentUuid`（链表结构，支持分支回溯）。实测一对
  tool_use → tool_result 落在 `02:11:25.953Z` 与 `02:11:27.202Z`——**相隔 1.2s 各写各的行**，
  不存在"回合末统一落盘"。中断的工具调用补一条 `[Request interrupted by user for tool use]`。
  每行都带 `isSidechain` 字段（子 Agent 消息的槽位；本机这些会话没用子 Agent，全是 false，
  **所以只能说格式留了位置，不能据此断言它一定落库**）。
- **Codex**（`~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`）：同样 append-only、一条一行、
  逐条带时间戳；`function_call` 与 `function_call_output` 相隔 **84ms** 各写各的。
  另有 `session_meta` / `turn_context` 记模型与工作目录。
- **两家都没有"继续上次的会话"提示条**：恢复是用户显式发起的（`--resume` / `codex resume`）。
  它们花力气的地方是**让存档本身完整**，不是事后提醒。

结论：这次的修法与两家一致，不是自创。

### 做了什么

1. **`AgentLoop.run(on_message=...)` + `_LiveList`**：一个 append 时回调的 list 子类。
   `run` 里有十来处 `messages.append`，往后还会加——逐点埋钩子迟早漏一处，而漏掉的那处
   正好是"某类消息永远不落库"这种静默的坑。已包过就不再包（auto_test 修复轮会拿同一个列表
   再调一次 `run`，重复包等于每条落两遍）。
2. **`_run_turn` 接线**：`_on_msg` 立刻 `history.append` + `_persist`；收尾只补尾巴
   （`rest = new_msgs[len(live_persisted):]`），没接上时退回原行为。
3. **`repair_interrupted_tail`（`context.py`，纯函数）**：见下。
4. **`resume_hint_minutes` 默认 0**：用户判定"意义不大"——它只记得住一个任务，
   而会话本来就带任务清单和笔记。代码与测试保留。

### 关键决策

1. **必须配套修 tool_use 配对**，否则这个修复自己会造一个更糟的 bug：断在
   "assistant 发了 tool_use、tool_result 还没落库"时，历史喂回模型会被 API 打回
   （`tool_use ids were found without tool_result blocks`）——"崩溃丢内容"变成
   "这个会话再也发不出消息"。补一条注明中断的 tool_result，**不删那条 assistant**：
   删掉等于把"当时打算干什么"一起抹了，而那正是接着干最有用的线索。和 Claude Code 一致。
2. **只看末尾**：历史中间的配对由 loop 保证，中间不配对是另一回事，别在这里顺手改。
3. **落库失败不许把这一轮带走**：`_LiveList.append` 里回调整个包在 try 里。
   写不进去是次要问题，任务白跑不是。

### 自检

- 新增 `tests/test_durable_turn.py` 6 条：发生即上报、回调抛异常不影响列表、切片行为不变、
  多个 tool_use 都要补、健康历史一个字不动、纯文本结尾不误判。
- 全回归：Python 全部测试文件 0 失败、前端 153/153。

### 段二：子 Agent 的检索也落库（`messages.sidechain`）

同一个洞的另一半——`run_subagent` 只把摘要回灌主 Agent，中间过程既不进 history 也不进库，
而委派型调研里搜索量最大的恰恰是这部分。做法对齐 Claude Code：**同一张表、打标记区分**。

要害是**读回来的那一侧**：`get_messages()` 默认不给旁链。混进主历史就等于把子 Agent 的
中间过程塞回主模型上下文，委派"独立上下文、只回摘要"的意义当场没了。落库只为两件事：
崩溃后还在，以及 `recall_history` 搜得到（模型据此不必重复搜一遍）。

顺带改对了两处容易漏的：
- `truncate_messages_after` 的 `keep` 是调用方按**主历史下标**给的，而旁链与主链交错躺在
  同一张表里——按行号直接切会砍错位置。改成先按主链定分界 id，再把它之后的全部删掉
  （被丢弃那几轮的旁链一并带走，不留孤儿）。
- 旧库没有这一列，`_migrate` 就地补上，历史照常打得开（有用例钉着）。

### 段三：「继续上次的会话」整块删掉

用户判定意义不大：只记得住一个任务、而会话本来就带任务清单和笔记。后端判定、前端提示条、
样式、配置项一并删除；`test_resume_hint.py` 里的换手落盘用例移到 `test_handoff_snapshot.py`。

### 遗留

- 无。三段全回归绿、真机验证通过，随 v3.77.0 定版。

---

## 2026-08-27 — 重连 MCP 把在飞的那一轮打死 + 会话提示错报

**状态**：✅ 已验证通过（v3.77.0 定版）。Windows 真机验证通过——跑着任务时点
「切到有头」，之后 browser_* 仍可用。

### 起因：用户说"委派子任务好像不如之前了"

先查的是提示词。把 `delegate.py` 与 `config.yaml` 里 `system_prompt` 的每个历史版本抽出来做指纹比对：
**委派相关的提示词自 v3.63.0（`bf42eda`，2026-08-12）起逐字未改**，v3.63.0→v3.76.1 八个版本
同一个 md5。所以"变差"不是提示词退化，是运行时在**悄悄失败**——顺着这条线挖出两个。

### 一、重连 MCP 之后，正在跑的那一轮一路「未连接」到底

用户现场：换手中途点了「切到有头」，之后每次 `browser_*` 都报
`MCP server 'browser' 未连接`，`web_fetch` 的浏览器兜底同样失效，模型据此写下
"这属于浏览器服务故障"——而用户看着设置面板说"我这边 MCP 连接是正常的"。**两边说的都是实话。**

根因是**对象生命周期**，不是连接：
- 重连（`Api._reconnect_mcp`）＝ close 老 manager + 建新的 + 重建各对话 registry；
- 但**正在跑的那一轮握着的是回合开始时快照的 registry**（`conversation.py` 里
  `registry = self.registry...` 那句），里面的 `McpTool` 在 `_serve` 里就把
  `caller=self.call` 绑死在**老 manager 的 bound method** 上；
- 老 manager 的 `close()` 清空 `_sessions`、置 `_loop=None` → `call()` 第一句就抛"未连接"，
  **直到本轮结束**；
- 而面板的"已连接"读的是 `res.mcp_tools`（新 manager 的），`errors` 只记**首次连接**失败，
  所以那边一切正常。

修法是**交班**而不是重连：`McpManager.succeed_to(new)`，老 manager 在 `call()` 里发现自己已经
没有 session 时，沿交班链找到还在跑的那个转过去（owner 一并透传，「停止」仍认得出人；
链带环保护）。没有接班人时行为一个字不变——那才是真的没连上。

### 二、「继续上次的会话」在用户自己新建会话时也冒出来

`pick_resume_session` 的三条判据只看得见"当前落在空白会话"，分不出这是重启后被动落进来的、
还是用户点了「新建会话」。于是跑着跑着新建一个，就被提示"继续上次的会话"——而上次那个
正是刚才那个。改成**一次启动只判一次**（`Api._resume_checked`）。

### 关键决策

1. **闸放在调用方，不放进纯函数**：`pick_resume_session` 保持纯（好单测），"进程是否刚启动"
   是状态，留在 `list_sessions` 里。
2. **交班而不是让老 manager 自己重连**：老的自己重连会**再起一个 Playwright MCP 子进程**，
   跟新 manager 那个抢同一份持久 profile。转发给已经在跑的那个才是对的。
3. **没接班人时不改变行为**：真的没连上就该报没连上。这个 bug 的教训不是"未连接这句话是错的"，
   而是"面板和调用看的是两个不同的对象"。

### 自检

- `tests/test_p6_mcp.py` +2：交班后调用转给新 manager（含 owner 透传）、交班链成环不死循环。
- `tests/test_resume_hint.py` +2：只在本次启动第一次提示；启动时已在会话里也算判过。
- 全回归：Python 全部测试文件 0 失败、前端 `node --test` 153/153。

### 遗留

- **面板的"已连接"仍是启动快照**：server 子进程**自己崩掉**（不是重连）时，`_sessions` 里
  没有它，但面板照旧显示已连接。要治得让面板读实连状态（`manager._sessions`），
  外加"session 掉了就自动重连那一个 server"。本次没做——先把已确认的那条修干净。
- **提示词回退到不了用户机器**：`config.yaml` 是**用户可编辑文件**，分发包只带
  `config.default.yaml`，更新解压**不覆盖**已有的 `config.yaml`（见 `load_config`）。
  所以仓库里把系统提示词回退掉，用户机器上那份**不会跟着回退**。

---

## 2026-08-26 — 浏览器穿透：没装 Chrome 的死路 + 重启丢成果

**状态**：✅ 已验证通过（v3.77.0 定版）。Windows 真机验证通过。

### 用户报的现场（四步，每步都缺一个反馈）

开了浏览器穿透但没勾有头 → 遇登录墙 → 面板提示「切到有头并打开这页」→ 点了，**浏览器没弹出来**
（本机没装 Chrome）→ 只能自己补一句"浏览器未弹出"再点「我做完了」→ 装好 Chrome 后重试**仍**失败
→ 模型建议重启 → 重启后**之前的检索成果全丢**，得重搜。

### 根因（四个叠在一起）

1. `browser_mcp_args()` **写死 `--browser chrome`**。没装 Chrome 必然起不来，而 `msedge` 本就是
   Playwright 合法 channel、Windows 一定有 Edge——这件事根本不必让用户去装。
2. **没有任何一环在看浏览器到底在不在**：切有头只 `_reconnect_mcp()`；`browser_mcp_done` 按
   「工具数 > 0」判成功——**MCP 连上 ≠ 浏览器能起来**（同类坑此前踩过一次：装好 23 个工具，
   `browser_navigate` 报 chrome-for-testing not installed）。
3. **失败只到了 UI、没到模型**。`handoff_open_target` 的错误回给前端 `showToast` 弹一下就没了，
   模型毫不知情地按"人已经去登录了"继续走——它手上没有任何能判断"为什么没弹出来"的事实，
   只能猜，而猜出来的出路代价最大（重启应用）。**模型看不到的失败，等于没有发生过。**
4. **重启即丢**：换手请求只活在内存里（db 里一条 handoff 都没有），且启动时
   `self.active = self._make_conversation(None, [], None)`——**每次启动都是全新空会话**。
   旧会话其实还在库里、能列出来，但用户落在空白页面上，看着就是"全丢了"。

### 做了什么

- 新 `browsers.py`：探测本机 Chrome/Edge（纯逻辑，路径存在性由调用方注入，可脱环境单测），
  **Chrome 优先、没有就用 Edge、都没有明确报错并给出路**。顺带修了"用户级安装的 Chrome 被
  当成没装"这个隐性误判。
- `handoff_open_target()` 切换前先探测；失败时经 `Conversation.notify_system()`
  把原因**作为事实送进模型上下文**（复用后台进程回投那条已验证的 enqueue 通路，
  忙则并入当前轮），并明说"不需要重启应用"。
- 前端失败信息**留在换手条上**（多行、按钮变「重试」），不再是转瞬即逝的 toast。
- `_snapshot_handoff()`：换手时把目标 + **本轮执行证据**（复用 `delegate.summarize_activity`）
  写进工作区 `.hermes/handoff/`。
- 「继续上次的会话」提示条（`agent.resume_hint_minutes`，默认 60，0=关）。

### 三个关键决策

1. **沉淀落工作区文件，不落 session_notes。** notes 要回到同一个会话才看得见，
   而这条路上最常见的恰恰是"重启后落在新会话里"。工作区文件跨会话、跨重启都在，
   模型用现成的 read_file / grep_search 就能找回来，**不必为它造新机制**。
2. **靠模型自觉写 notes 是不够的**——它想不起来的时候正是最该记的时候。所以由代码做。
3. **只提示、不自动切回上次会话。** 自动切会让"我就是想开个新的"每次都要先关掉一个旧的；
   而且规则要窄——只在"落在空白新会话 + 上一个会话阈值内还在动"时才出现。

### 自检

新增 `tests/test_browsers.py` 6 条、`tests/test_resume_hint.py` 8 条、前端 2 条；
改 `tests/test_offsite_wait.py` 一处 fake（`_on_external_event` 拆出 `notify_system` 后要多借一个方法）。
全回归 Python **93/93**、前端 **153/153**。

### 遗留

- **Edge 回退后的穿透效果与登录态持久化没验过**（Linux 上验不了）：要在 Windows 上真跑一次
  ——弹出的是不是 Edge、登录态是否存进独立 profile、下次浏览是不是已登录。
- 换手请求本身仍不跨重启（只有成果落了盘）。真要做得把 handoff 落库，另议。

---

## 2026-08-26 — 委派并行的账算平（同回合检索缓存 / 收敛判据 / 子任务墙钟 / 拆分判据）

**状态**：❌ **提示词部分已整块回退**（2026-08-26 真机使用后用户判定质量下降）。
本条保留下来是因为**那三轮真跑的数据与教训仍然有效**，但结论别再照着改提示词——见文末「回退」。
缓存与子任务墙钟两个开关**默认关**，行为回到 v3.76.1。

### 起因：一次把"感觉慢"变成数字的真跑

用户反馈"多个调研并行同时跑，整体反而比主模型自己搜慢"。写了 `scripts/diag_delegate_speed_realrun.py`
在开发机真跑 A/B（同一道题：调研 2026 年本地部署 70B 大模型的装机配置），在 provider 那层拦时间线。
**结论：机制没坏，是账算不过来。**

| | A 主模型自己干 | B 委派并行 |
|---|---|---|
| 模型调用次数 | 24 | 57 |
| 模型调用累计时长 | 506.6s | **1650.9s（3.26×）** |
| 工具调用 | 19（主） | 46（主 5 + 子 41） |
| 真实并发度 | 0.84 | **2.55（峰值 3）** |
| 限流退避 | 0 | 0 |

**并行度 2.55 < 工作量膨胀 3.26 → 净亏。** 同轮并行派发本身是好的：三个 `delegate` 与三个
`subagent_start` 全部落在 41.3s 同一时刻，`loop.py` 那条「同轮 ≥2 才并发」的路径走通了。

当时列了膨胀五源（**②③④⑤都能在时间线上直接指出来，只有①是纯推断**——后面被证伪的正是它）：①子 Agent 互不知情、重复搜同一片领域；
②子 Agent 会把步数用满（sub#3 正好 15 步、sub#1 到 614s 还在 fetch）；③木桶效应
（三个 done 落在 537.6/629.7/646.4s，白等 109s）；④汇总是 A 完全不用付的一大轮；
⑤委派本身的固定启动成本（41.3s 才把三个 delegate 发出去）。

**口径要说清**：那次两段都撞了脚本设的 600s 上限、都没跑完，所以"慢 1.08×"**不是完成时间的对比**，
有效的只有工作量那几行。

### 第二、三轮：把推断打回原形

四条改动里的第 1 条（检索缓存）就是照推断①做的。第二轮（cap 1200）缓存**命中 0 次**，
但日志只有工具名，判不出是"键太严"还是"根本没重复"。于是第三轮给脚本加了
**查询词打印 + 近似重复分析**（用 `web.py` 自己的分词算 Jaccard，别另发明一套），换小题、cap 1800——

**两段第一次都完整跑完**（A 99.2s / B 169.7s，委派亏 1.71×），查询词直接把推断证伪：
9 次搜索是 **9 个不同查询、逐字重复 0 次**，近似重复只有 1 对（0.45，还是 sub#1 自己跟自己）。
三个 researcher 分工其实很干净：速度基准 / lockfile 与依赖解析 / 生态替代。

**真正的膨胀是结构性的**：每个子 Agent 都要跑一遍完整的 agent 循环，外加汇总那一轮——
单次模型调用耗时两段几乎一样（12.5s vs 13.2s），膨胀纯粹来自**次数**。

三轮合起来给出真正的判据：

| 轮次 | 题 / 上限 | 单次检索 | A 段 | B 段 | 结论 |
|---|---|---|---|---|---|
| 1 | 70B 装机 / 600s | ~30-60s | 600.1s 截断 | 646.3s 截断 | 不可比 |
| 2 | 70B 装机 / 1200s | ~90s | 1200.1s 截断 | 615.9s 跑完 | 委派赢 ≥1.95× |
| 3 | uv vs pip / 1800s | ~20-40s | 99.2s 跑完 | 169.7s 跑完 | 委派亏 1.71× |

**委派赚不赚 ≈ 串行做完所有检索要多久 vs 并行度能不能摊掉 N 倍循环开销。**
检索又多又慢就拆，少或快就别拆——**跟子问题重不重叠关系不大**。

**教训（值得记住的那条）**：从"次数比"推因果是不够的。**排查性能时，判据本身也要打点**——
第三轮多打的那几行查询词，比前两轮所有的时长数字都值钱。

### 做了什么（按性价比依次，①最要紧）

1. **同回合共享检索缓存**（新 `webcache.py`）——**原以为治膨胀的大头，实测治的是不存在的病**。主 Agent 与所有子 Agent
   **共用一个实例**（照 `ToolBudget` 的先例，各拿一份等于没做）；按回合清空；同 key 单飞；
   命中时在结果里明说并让模型换角度。**保留它的理由**：正确、零开销（不命中就是一次字典查找）、
   有 13 条测试兜着，真出现重复时确实省；**但不当卖点**，也别为提高命中率做模糊匹配。
2. **researcher 收敛判据**——`subagent_max_steps` 本来就有（默认 15），缺的是"够了就收"的判据：
   每个子问题 2~3 个独立来源即停、连续两次没新事实即停。
3. **子任务墙钟上限**（`agent.subagent_timeout_s`，默认 900s）——治木桶效应，复用撞步数上限
   那条收尾路径。
4. **拆分判据写进 delegate 说明与系统提示词**——**先估自己做要几次检索、少于 5 次别拆**；
   要并行必须同一轮发多个。**这是三轮数据唯一共同支持的一条**。

### 三个关键决策

1. **缓存按回合清空，不跨回合。** 跨回合复用会把"再搜一下最新的"变成拿旧结果糊弄——
   陈旧结果比多花一次搜索危险得多；而回合内的重复搜索是纯浪费，清掉零代价。
2. **只归一化、不做模糊匹配。** 把"意思差不多"的两个查询判成同一个，省下的那次搜索换来的是
   模型拿到答非所问的结果，而它查不出为什么。`focus`/`max_chars` 也必须进键。
3. **撞时间上限不杀线程。** 一个跑了十分钟的子任务被硬杀掉，等于十分钟白花；
   让它禁掉工具把已有成果总结交回、并**明确标注未完成**，主 Agent 才知道该补什么。
   判定只在步与步之间（同 cancel 的契约），不打断在飞的调用。

### 自检

- 新增 `tests/test_webcache.py`（13 条）：命中/不串味/失败不缓存/回合清空/淘汰/
  单飞三态（复用、leader 失败、等待超时）/主子共用同一实例/开关关掉即回到旧行为。
- 新增 `tests/test_delegate.py` 4 条：墙钟收尾走同一路径且两个标志不混、0=不限、
  两处提示词判据钉死（防"精简提示词"时被清掉）。
- 改 `tests/test_firecrawl.py` 一条断言：搜索主体从 `run` 挪进 `_search`，
  该测试盯的是"工具没有自己另写一份渲染"，故改看整个类。
- 全回归：Python **91/91 文件 0 失败**，前端 **151/151**。

### 回退（2026-08-26 当天）

真机用下来**质量下降**，用户要求恢复。回退的是**提示词**（`delegate` 说明、researcher 指令、
`SUBAGENT_DIRECTIVE`、`config.yaml`），逐字回到 v3.76.1；缓存与墙钟两个开关默认关。

**根本错误**：给模型加了一条「**先估自己做要几次检索、少于 5 次就别拆**」的**自我估算门槛**。
模型估不准——调研类任务开搜之前根本不知道要挖多深——于是把本该多角度并行调研的任务
当成小活直接开干，连带也不再加载调研技能。**而用户举的反例（本地部署大模型的装机配置调研），
正是这三轮里委派赢的那一道**（串行 1200s 没收完 / 并行 616s 给出完整答案）：
我拿另一道小题（uv vs pip，2~3 次检索）的阈值当了通则。

同批的 researcher「拿到 2~3 个独立来源就停止检索」也与该角色原有的
「逐层下钻、别只给一级摘要」直接打架，会让子 Agent 变浅。

**教训（比那三轮数字更值钱）**：
1. **别把一道题的实测阈值写成通则**，尤其是需要模型**自我估算**才能用的门槛——
   它估不准的地方，正是这条规则最要命的地方。
2. **提示词改动的验收不能只看回归绿**。回归测的是代码，提示词的效果只有真用才知道；
   这次是用户在真实任务里发现的，不是任何一条自动化测试。
3. 数据支持的是"拆得不好会亏"，**不是**"这类任务别拆"。两者差一个字，方向差 180 度。

### 遗留

- **缓存的收益仍未被任何一轮真跑证实**（两轮命中 0），现已默认关。
- 缓存目前只在**同回合**生效。跨子任务的"发现共享"（一个子 Agent 找到的结论让别人看见）是更大的
  一步，没做。
- 第 ④ 条（汇总那一大轮）与第 ⑤ 条（启动成本）**没治**——它们是委派语义自带的成本，
  只能靠"该不该拆"的判据规避。

---

## 2026-08-25 — v3.76.0：hooks 事件扩容（UserPromptSubmit + Stop）+ read_file 报总行数

**状态**：✅ 已完成并**定版 v3.76.0 → v3.76.1**（后者修发版当轮逮到的回放门红，见文末）；
Python 全回归 88 个测试文件 0 失败、前端 145/145、三层回放门 19/19、Golden 67/67。**改动全在后端、无 GUI 变更**，因此没有 Windows 真机主观项——Windows 侧由 CI 的
windows job 跑全回归覆盖。

### 起因：一次"同题实跑"顺带把待办做了

为回答"hermes 目前的水平相比主流 agent 如何"，拿 ROADMAP 第三档第 8 条的 **B3** 当同题，
让 hermes（无头 `run_cli.py` + deepseek-v4-flash）与 Claude Code（Opus 5）各做一遍：
同起点（v3.75.1 干净副本）、同验收判据（跑前写死 C1–C10）、同一把只走公共面的探针（探针自身验过活性，
对未改动的原树只给 3/8）。结果两边都 **8/8 判据 + 回归全绿 + 新测试有活性**，解法几乎撞车。
本次合入**以 hermes 那份为底**（它多同步了 `config.yaml` 与 `api.py` 的注释），再补两处。

### 做了什么

- **`UserPromptSubmit`**（用户消息进内核前）：退出码 **2=拦截本轮**（不落库、不建会话、不调模型、
  不触网），**0 且 stdout 非空=作为附加上下文注入本轮**。挂在 `send_message` 里 `save_images` **之前**——
  被拦的那一轮不该在工作区留下副作用。注入走 `_append_prompt_context`，兼容 str / blocks 两种 content 形态。
- **`Stop`**（一轮终态）：payload 带 `reason ∈ {done,error,stopped}`，**stdout 不回灌模型**。

### 两个关键决策

1. **四个终态出口收成一个 `_fire_stop()`**。`_run_turn` 有四条 return 出口，原补丁只挂了三条——
   **漏的正是 `build_provider` 失败（模型档/密钥配错）那条**，实测确认 Stop 不响。四个出口各写一遍的
   写法，漏一个不会有任何测试发现。现在 `hook_runner` 提到 provider 构造**之前**建好（否则那条出口
   没得用），四条出口统一走收口函数。**这条 bug 是本次实跑的直接产出。**
2. **crazy 自驱轮不触发**（用户 2026-08-25 拍板）。`_run_crazy_round` 走 `_run_turn(fresh=True)`，
   一个自驱任务动辄十几二十轮，每轮响一次会把「Stop 时自动跑测试/发通知」变成跑二十遍——
   那不是用户配这条 hook 时想要的东西。`fresh` 只有 crazy 一个来源，所以按它开关刚好精确。
   `UserPromptSubmit` 同样不进 crazy（轮次目标是模型自己生成的，不是用户说的话）；
   **「重新生成/编辑重发」也不触发**（走 `_RERUN` → `_run_turn`，那不是新的用户输入）。
   **代价写在这里**：整个 crazy 任务结束时也没有 Stop——任务级收尾是另一件事，要做另开。

### 自检

- `tests/test_hooks.py` 22（+9：新事件匹配、`parse_prompt_result` 各退出码、拦截/注入/payload 落盘、
  stop payload+reason、两个事件的零开销）；`tests/test_config_keys.py` 18（+1 白名单）；
  `tests/test_conversation.py` 106（+5：拦截发生在 `build_provider` 之前、`_append_prompt_context`
  两形态、**第四个终态出口**、**crazy 不触发 Stop**、**crazy 不触发 UserPromptSubmit**）。
- **活性验过**：去掉 `_fire_stop` 里的 `or fresh`，`test_stop_hook_not_fired_in_crazy_round` 当场变红。
- 中立探针（`config._norm_hook` + `Conversation.send_message`，不碰内部命名）**8/8**。

### 同版另一件：`read_file` 没读完时报「本文件共几行」

起点是同一次实跑的旁证：hermes 那 60 次工具调用里 **`read_file` 占 25 次，全是 45–120 行的小窗口**，
而 `MAX_READ_LINES` 其实是 2000。查下去发现**工具从不告诉模型这个文件有多大**——只在没读完时
提示下次 offset。模型没有"这文件才 300 行，一次读完得了"的判断依据，只能一段段试。

- **只在没读完时报**：读到末尾了不加尾巴——那时最后一行的行号本身就是总数，再说一遍是纯噪音。
- **工具说明里把用法一起写了**（"据此判断是接着读完，还是该改用 `grep_search`/`code_outline`"）：
  **光给数字不给用法，模型多半照旧一段段翻。**
- 数行**复用同一个已打开的句柄**往下数（只数行、不留内容），不重读文件；封顶
  `MAX_COUNT_LINES=200000`，超了报 `200000+`——别为一个数字读穿 100MB 的文件。
- 测试 +2（部分读报总数 / 读完不报；封顶报 `N+`，用临时改常量测，不写 20 万行文件）。

### 发版当轮逮到的一条（v3.76.1）：改工具说明＝改请求指纹

推 tag 后 `release` 绿、`ci` 红。红在块 V3 离线回放：`bugfix` 报「第 1 次模型请求没有对应录音
（key=d154490c…）」。**不是代码错，是录音失效**——我给 `read_file` 的 description 加了一句用法提示，
而**工具 schema 属于请求的一部分、进 cassette 指纹**，改一个字整套录音就对不上。

本地复现→只还原那段描述→L1 立刻 PASS，病根定死。**没有选择重录**（重录要真模型跑一遍所有任务，
还会把对比基线一起换掉，代价远大于收益），而是**把提示挪进结果尾巴**：

- **时机更对**：只在真读了一半时才说，不必每一轮都占 schema 的篇幅；
- **不动指纹**：录音继续有效。尾巴属于工具结果，只在部分读时出现，评测夹具文件小、读得完，
  所以对回放零影响（三层 19/19 验过）。

**教训（已写进 `fs.py` 注释）：工具的 description 不是"注释"，它是请求内容**。
动它等价于动 prompt——要么接受重录，要么把话写到别处。

### 遗留

- **`PreCompact` 未做**（B3 三个缺口的第三个）：压缩前抢救关键状态，得先想清楚"抢救下来的东西放哪"。
- **GUI 设置面板的事件下拉仍只有两项**（`web/app.js`）：新事件目前只能从 `config.yaml` 或
  `user_hooks.json` 配。加下拉会牵动前端 DOM 与 `api.py` 的 hooks 管理面，单独排。
- **crazy 任务级 Stop**（见决策 2 的代价）。
- **hooks 这个面要不要继续投入，定了条线**（2026-08-25 与用户讨论后的判断）：hooks 最常见的三个
  用法（写完校语法 / 收尾跑测试 / 收尾派人审）**产品早已内置**（`auto_verify`/`auto_test`/`auto_review`，
  `hooks.py` 的 docstring 本来就说它是这三者的泛化），所以作者自己几乎用不上是合理的。
  **若三个月内一条 hook 都没配过，就不再往这个面上投入**——当"已交付、维持现状"，
  别再当"小而确定该先清"的待办推荐。GUI 设置面板的事件下拉同理，暂不做。
...
✅ **Windows 真机验证通过（2026-08-25）**——黑框不再闪。
...
**真跑验证 16/16**（`scripts/diag_stop_realrun.py`，真起 stdio MCP server + 真出网）。
✅ **Windows 真机验证通过（2026-08-25）**：按下停止后联网搜索确实停得下来（见文末）。

### 起因：同一个停止键，该停的没停、不该停的停了

用户真机上两条现象同时出现：A 会话点停止，A 自己的联网搜索照跑不误；而**另一个**会话 B
正在跑的 codex 反倒当场被打死。看着像一个 bug，其实是两半，病根完全不同。

### 第一半：取消不认人（已修）

`McpManager` 是**跨对话共享的单例**（`api.py` 里 `Resources(mcp=mcp, …)`），而在飞调用记在
一个全局 `set` 里，`cancel_all()` 无差别取消**所有** future。所以"停 A"＝取消整个进程的在飞
MCP 调用。而 A 跑的 `web_search` 根本不走 MCP——**用户看到的两个症状是同一个 bug 的两面**。

改法：`_inflight` 从 `set` 改成 `{future: owner}`，owner 是对话 cid，由 `McpTool.bind_workspace`
在建注册表时绑（绑成副本——同一批工具实例被所有会话共用，直接改字段会串台）。
`cancel_all(owner)` 只取消该对话的**外加无主的**：没绑 owner 的调用没有别的出口，
**宁可多停也不能留下停不掉的活**；不传 owner 仍是全停，留给关停/退出。
子 Agent 的注册表绑同一个 cid——它的调用算发起它的对话的，停本对话要连它一起停。

### 第二半：阻塞型工具压根没有检查点（本次新增）

`stop()` 的契约是"回合间生效"，不打断回合内的工具执行。而一次 `web_search` 最坏要串上
firecrawl → bing rss → bing html → ddg → 读 3 条正文，**每跳一个 timeout（默认 20s）**，
中间一个检查点都没有——按了停止只能等好几个 20s 串完，用户视角就是"按钮坏了"。

**立场：不打断已发出的请求，只掐掉后面的回退。** urllib 没有取消接口，硬断连接是另一类麻烦；
而真正的时间都花在"这条不行、换下一条再试一次"上。于是在每一个**换源点**查一次令牌：
工具入口 / bing 的 RSS→HTML / 主源→免 key 链路 / 升级闸 / 读正文 / web_fetch 的
HTTP→Firecrawl→浏览器三档之间。最坏等待从"几个 20s 串起来"压到"当前这一跳"以内。

配套两条规则（都写进了注释和用例）：

1. **有货就回货、没货才报停**。停止落在拿到结果之后，那批结果——尤其是花过 credit 的
   Firecrawl 结果——照常给出去，只是不再往下读正文，并在结果里说明「已停止，未再读取正文」。
   因为"用户按了停止"不等于"已经拿到的东西作废"。
2. **说清是"没试"而不是"试了没成"**。web_fetch 的兜底阶梯被停止跳过时，错误里明写未再尝试；
   否则排查时会一头扎进一条根本没跑过的路（同 v3.74 那条"没用上付费源也要说出来"）。

令牌接受 `threading.Event` 与无参 callable 两种形态（本项目两种都在用），`None`＝没接、行为同 3.74；
令牌自身抛异常按"没停"处理——**取消机制自己出问题不该把工具带崩**。

### 自检

- `tests/test_web.py` +8 测（令牌语义三形态 / 入口拒绝 / bing RSS→HTML 之间停 /
  有货就回货 / 跳过读正文 / web_fetch 不再爬阶梯 / 受阻页照常回内容 / registry 真的接上了令牌）；
  `tests/test_p6_mcp.py` +2 测（停 A 不动 B 的 future / owner 真的传到了 manager 且不污染共用实例）。
- **活性验过**：把 `is_stopped` 临时改成恒 False，7 条新用例逐个变红（registry 接线那条另有守）。
- **真跑** `scripts/diag_stop_realrun.py` **16/16**：
  - 段 1 真起 `mcp_echo_server.py`，两个 owner 各调一次 `sleep(30)` 并发在飞，
    `cancel_all("A")` 后 A **0.0s** 收到「调用已被用户停止」、**B 仍在跑**，再全停 B 才结束；
  - 段 2 真出网搜同一个查询：基线 2.6s 且读了正文；停止落在搜索途中时 **0.9s** 返回、
    结果照给、没读正文；停止后排队进来的调用 **0.000s** 报停、一个字节都没出网；
  - 段 3 真连 discard 端口：不带令牌时浏览器兜底真的被走了一遍，带令牌时 0.00s 报停、兜底没发生。
- 端到端确认接线：真建一个 `Api`/`Conversation`，`web_search._cancel is conv._cancel` 为真，
  `conv.stop()` 后令牌置位——**注入不上等于没做**。

### 顺手修的

`scripts/mcp_echo_server.py` 在 mcp 2.x 上起不来（2.0 把 `mcp.server.fastmcp.FastMCP` 改名为
`mcp.server.mcpserver.MCPServer`），子进程 `ModuleNotFoundError` 而客户端只看到 "Connection closed"。
两版都认。同时给它加了个 `sleep` 慢工具——**没有慢调用就复现不出归属问题**。

### 遗留

- ✅ **Windows 真机一条主观项已验（2026-08-25）**：界面上按下停止后，正在跑的联网搜索
  确实停得下来（本机测到的是"当前这一跳以内"，真机网络更慢，用户亲眼看过确认）。
- **A 自己那半的更彻底做法没做**（把网络调用挪进可取消 future、`stop()` 直接 cancel）：
  能连已发出的请求一起断，但要动 web 工具的执行模型、容易牵出连接泄漏。当前这版是
  "改动小、语义干净"的一档，先看真机效果再决定要不要往下走。

---

## 2026-08-21（晚）— v3.74.0：「🔍 联网检索」设置页 + 枚举型设置机制

**状态**：✅ 已完成并定版 v3.74.0；本地全回归全绿（Python 87 个测试文件 0 失败、前端 145/145）；
**✅ Windows 真机验证通过**（用户按 `hermes-dev-3.73.1dev-联网检索面板-测试用例.md` 的 A–E 全清单跑完）。

### 起因：一次排查困境，暴露的不是 bug 是产品缺口

Firecrawl 要生效得**两个条件同时成立**——`FIRECRAWL_API_KEY` 从环境读、`web.firecrawl` 档位在
`config.yaml`。而任一边缺了，**表现完全一样**：搜索照旧走 bing，一声不吭。
更糟的是 `.env.example` 和 `pack.py` 的打包模板里**根本没出现过这个变量名**——
"这把 key 配在哪"在产品里查不到出处。

**所以这一页的第一职责不是"能填"，是"说破"**：面板与左栏徽标报的是**实际生效档位**，
不是配置里写的那个。没 key → 「缺 key」，配额用尽 → 「配额用尽」，两种都直说
"现在实际走的是免 key 的 Bing + DDG"。这是「动用付费源要说出来」那条纪律的反面：
**没用上付费源，同样要说出来。**

### 三个设计决定

1. **key 与档位同屏**。分开放就是把"两个条件都要对"的负担丢给用户，正是这次踩坑的形状。
   填完写回 `<安装目录>/.env` 并即时生效——不重启、不手编文件（测试用例 B4 是这条的唯一证明）。
2. **换 key 顺带清「配额用尽」的进程内粘滞标记**。"配额用尽了换把 key"正是会来这一页的
   主要动机之一；不清则新 key 当场又被当成已用尽——修好了却像没修。
3. **落成通用机制而不是一次性旋钮**：`CHOICES_SPEC` / `choices.json`，与既有 `LIMITS_SPEC` 同构，
   只是值域为有限候选。**既有两个机制都塞不下**：`feature_flags` 只覆盖 `agent` 段且值是布尔、
   `limits` 只吃数字区间；硬塞"三四个具名档"会把两边语义都搞浑。
   脏值与白名单外的 key 一律跳过、绝不落盘；`config.yaml` **不被改写**（用户手编的注释不会丢）。

**Key 只回掩码、绝不回明文**——延续设置面板既有立场，`test_pane_never_leaks_plaintext_key` 钉住。

### 自检

- `tests/test_choices.py` 15 测（spec 形状 / 读写往返 / 脏值与未知 key / 大小写空格归一 /
  面板实际档位三态 / 不泄明文 / 换 key 清粘滞标记 / 前后端选项一致）。
  其中 `test_firecrawl_options_match_backend` 是**跨层闸**：前端下拉选项与后端白名单对不上就红。
- `tests/web/web_search_badge.test.js` 6 测（徽标四态：off 不喊 / 缺 key / 配额用尽压过已生效 / 绿灯）。
- 全回归：Python `for t in tests/test_*.py` **87 个文件全绿**；前端 `node --test` **145/145**。

### 遗留

- `.env.example` 与打包模板补的是**注释行**（`FIRECRAWL_API_KEY=` 留空），不预填任何值——
  与"开箱不预设 provider"一致；同时标明它是**检索源、不是模型 provider**，没有也能用。
- 枚举型设置目前只有 `web.firecrawl` 一个成员，机制的第二个用例出现前不再扩容。

---

## 2026-08-21（收尾）— v3.73.1：回放门可移植 + Firecrawl 档位 A/B 实测

**状态**：✅ 已完成并定版 v3.73.1；CI 连续两个提交全绿
（`7c5f732` run 32450716566、`93088d9` run 32452681623，均 `test` + `floor` 双绿）

**Windows 真机验证**：我原话是"这一版没有需要真机验的项"——**推 tag 时就被打脸了**。
`release` 的 Windows 门当场逮到两个**真机行为缺陷**：分类器不认 Windows 的"命令找不到"、
`fold_workspace` 只折一种分隔符形态。两个都不是测试的毛病，是产品行为在主力平台上不成立。

**教训比修复重要**：`ci` 只跑 ubuntu、Windows 只在推 tag 时跑，等于**主力平台最后才验**。
已给 `ci.yml` 加 `windows` job 前移这道门。另一条是方法上的——
前两轮我"改一条、CI 再红一次"来回三趟，第三轮才想起在 Linux 上先造个"假 Windows"
（`PYTHONPATH` 指一个 `sitecustomize.py`，在 `sys.meta_path` 里拦掉 `resource`）
一次把失败找全。**能本地复现就别拿 CI 当调试器。**
...
这一版的价值不在代码量，在**每条问题都先查清"是谁的限制"**：

1. **截图贴进对话 Codex 看不见** → 查 schema：它的 MCP 工具**没有图片入参**，
   CLI 入口是 `-i <FILE>`。**不是 MCP 的锅，也不是 ACP 能省掉的**（ACP 协议层虽允许内联图片，
   但要多一层第三方适配器且未验证）。结论：落盘是唯一通路 → 做落盘。
2. **"委派卡一分多钟还没开始干活"+ reconnecting** → 拆开量：本机 MCP 链路连接 0.7s、
   每次调用约 1s，**成本根本不在 MCP**。再查：hermes 进程里没有任何代理变量。
   但用户反馈"终端也偶尔断"——说明网络本身不稳，**hermes 改不了**；
   真正的差别是终端一个会话连着聊、hermes 每次新建会话，**同样的毛病终端摊薄、hermes 全额付**。
   能做的只有把链条摊开给人看 → 体检加代理检查。
3. **只读会话后续要改文件就卡死** → 查 schema：`codex-reply` 没有 sandbox，
   **沙箱在会话创建时定死**。代价不对称（先给写后来只看无所谓；先给只读后来要改就得推倒重来），
   所以默认该给写权限 → 规则写进工具说明，让模型第一次就选对。

**三条的共同点**：都先用探针/schema/计时把"谁的限制"定死，再决定改哪儿——
不然很容易改错地方（比如为图片去换 ACP、或为重连去优化 MCP）。

---

## 2026-08-21（晚）— v3.72.1：把"在别处干活"这条路堵上

**状态**：✅ 已交付，端到端真会话验证通过，已定版 v3.72.1

真机用下来的一件事：Codex 自述"已创建 server.js/index.html"、还从那儿起了服务占住 3000 端口，
而 hermes 工作区里**一个文件都没有**。用户翻进程列表才发现。

问题不在"它写错地方"本身，而在**整条链路对此完全无声**：工具结果只有 agent 的自述；
`[本次改动·git status]` 只看工作区，它在别处写就什么都报不出来；而"没有改动"当时又是
**不显示**的——沉默叠沉默。

三道防线（用户问的那句"不能让 codex 直接在设置好的工作区跑吗"直接催生了第一条）：

1. **`cwd` 强制夹回工作区**（`clamp_cwd`）：区内子目录照用，区外改回根并说明。
   原先是"模型没给才补"——**给错一个目录就整场跑偏，且无人知晓**。
2. **回显 `[工作目录]`**：万一还是别处，一眼看见。
3. **"工作区无改动"说出来**：让"它说建了 + 工作区没有"这对矛盾当场可见；
   但测不了要保持安静，别把"没测"说成"没改"。

端到端：故意把 cwd 指到工作区外 → 被夹回、文件落在工作区、区外目录空的、改动块报出 `?? made.txt`。

---

## 2026-08-21 — v3.72.0 定版：把 Codex 当子 Agent（用 ChatGPT 订阅额度）

**阶段**：FR-6.4b（agent 型 MCP server）
**状态**：✅ 已交付，✅ **已在 Windows 真机验证通过（2026-08-21）**，已定版 v3.72.0

### 起点是用户的一个提议

"codex 不是也可以接 deepseek 的模型吗？你可以用 deepseek 配合 codex 做开发测试"。
这一句把最大的瓶颈拆了：此前 Codex 相关的一切都得等 Windows 真机（我这边没 ChatGPT 订阅）。
实测 `wire_api="responses"` + DeepSeek 的 `/v1/responses` 可用（0.148 起 `"chat"` 已移除），
本机从此能跑**真 Codex 会话**——审批通道、事件形状、mcp 1.x 的行为，全是这么探出来的。

### 接入本身几乎零代码，难的全在"用得下去"

`codex mcp-server` 是官方的 stdio MCP server，hermes 本来就会说 MCP。
但七项易用性改动里**六项来自真机撞出来的痛点**：不弹确认、黑箱、续话丢上下文、
在错目录干活、停不掉、连不上却查不出原因。

### 真机逼出来的四类"静默失效"（值得记）

1. **SDK 驼峰/蛇形**：`isError` 读不到 → **MCP 工具报错从来没被识别成错误**；
   `structuredContent` 读不到 → 续话 id 一直取不到。CLAUDE.md 早记过同类坑
   （`inputSchema`），但当时**只改了那一个**。教训：这类字段要全库搜一遍。
2. **权限优先级选反**：`trust` 与 `always_confirm` 同时为真时按 trust 走，
   于是"每次都问"被静默作废。**权限上宁可多问一次，也不能让两个开关打架的结果是谁都没拦住。**
3. **mcp 1.x 根本没有自定义通知的口子**：只 log 一句就丢。改成在读流上加旁路。
   我这边是 2.0.0，一直是好的——**版本差异是最容易漏测的一类**。
4. **子进程环境被 SDK 过滤**：代理变量全没了，于是 codex 在子进程里反复重连，
   而同一条命令在用户终端里好好的。

### 一处我自己造的坑

体检用 `p.stdout.readline()` 读子进程输出，**对端不吐字时永久阻塞**，
外面那个 `while time.time() < deadline` 循环压根没机会跑——按钮卡在「体检中…」不动。
本机的 codex 每次都秒答，所以我一直没撞上。已加硬看门狗 + UI 侧超时：**界面永远不该无限等**。

### 自检

全量在**两个 mcp SDK 版本上各跑一遍**（2.0.0 与 1.27.2）：Python 85/85 文件、
前端 139/139、Golden 67/67；3.11 编译通过；端到端真 Codex 会话逐项验过
（实时流 / 续话 / cwd 注入 / git 改动 / 取消）。

### 遗留

- **Codex 的审批 hermes 只能拒绝**："同意"该回什么值没能确定（真会话逐个试过）。
  出路：派活时给 `approval-policy="never"`，用 `sandbox` 定影响范围。
- 国内用 ChatGPT 订阅时每轮先重试 WebSocket 再降级 HTTPS（`Reconnecting… N/5`），
  Codex 自身策略 + 网络所致，功能不受影响；那两个 flag 已 removed、关不掉。

---

## 2026-08-20 — v3.71.0 定版：喂饱评估内核（V0–V5）+ 托管检索源（FR-11.1d）

**阶段**：第三阶段 V0–V5（ADR 0027）+ FR-11.1d
**状态**：✅ 已交付，✅ **已在 Windows 真机验证通过（2026-08-20）**，已定版 v3.71.0

### 做了什么

- **V0–V5**：把建好却在空转的评估内核接上输入——指纹归一 + 语料分库（V0）、Run Record
  与对比报告（V1）、任务集 6→26（V2）、录制回放进 CI（V3）、Learning 收割（V4）、
  detector 计分板与阈值扫描（V5）。CI 门 6 → 21，全程离线、不需要任何 secret。
- **FR-11.1d**：Firecrawl 做成引擎/兜底层（工具面零变化）。段 1 召回换源、段 2 读页兜底，
  降级阶梯 HTTP → Firecrawl → 浏览器。

### 真机暴露、本地与 CI 都没抓到的三件事

1. **`cassette.py` 的 3.12-only 语法**：`pyproject` 声明 `>=3.11`，而开发机与 CI 都钉 3.12，
   于是本地全绿、CI 全绿、**用户一启动就 SyntaxError**。
   补了 `floor` CI job（3.11 编译 + 全回归 + Golden）——**声明支持哪个版本，就得在那个版本上跑一次**。
2. **thinking 把重试保护踩没了**：`RemoteProtocolError` 本在瞬时清单里，但门槛是
   "yield 过任何事件就不重试"，推理模型先吐 thinking → 这层保护对推理模型等于不存在。
3. **"改的树 ≠ 跑的树"又踩一次**：用户报"全程没触发 Firecrawl"，查到最后是**日常用的安装
   根本不是这棵检出**（那个目录已不存在）。教训：这类反馈先验"跑的是哪一版"，
   别一上来就怀疑判据——现在自查的第一条就是打印 `FIRECRAWL_MODES` 与模块路径。

### 一处被真机推翻的设计结论

段 1 写过"默认 fallback 是算术不是偏好"（search=2 credits、免费档 1000/月）。
真机反馈：**装上 key 后全程没触发过**。算术没错，**前提错了**——省下的配额只有在
"免 key 链路真的会不够用"时才有价值，而实测它几乎总被判为够用，于是配额既没花掉、
也没换来更好结果，纯属闲置。用户拍板改默认为 `primary`（主搜），配额用尽自动退回并粘住。

### 自检

Python 82/82 文件、Golden 67/67、前端 133/133、离线回放门 21/21；3.11 与 3.12 双版本编译通过。

### 遗留

- `primary` vs `always` 的 A/B 还没跑（要先补 3–5 个真网检索任务，现在只有 1 个任务真的会碰它）。
- 三个哑着的 detector（stuck/search/deadend）仍缺能稳定施压的正例——**在没有触发样本的
  情况下调参，调的是噪声**。

---

## 2026-08-14 — 站外协同 W1：让 agent 自己醒来，并且记得当初在等什么（ADR 0026）

**起点是用户的一个观察**：别的 agent 能"推 tag 触发 CI → 说一句好了告诉你 → 会话空闲 →
跑完自动回来报结果，红了自己修"，问 hermes 具不具备。**查证结论：只缺四分之一步。**
①发起 ②说一句 ④醒来后自主处理 都已具备；③**零成本等待 + 唤醒回到同一上下文**完全没有
（`procs.py` 纯拉取、`_reader` 退出时谁也不告诉、全库无调度/唤醒、`state` 纯内存无 resume）。

**④ 看着最神奇，其实不是独立功能**——它成立只因为唤醒时上下文还在。所以要补的不是"通知"，
是"**带着上下文把这一轮接上**"。而 `_take_injects`/`enqueue` 这条缝天然往当前会话投，已经就位。

**用户点出真正用途后，设计变了**：*"有了它才能让 hermes 去调别的 agent 并知道站外进度"*——
调别的 agent 是**本地长跑进程**，根本不需要轮询，只要"它退出时告诉我"。于是做成两种形态：
`notify_on_exit`（进程退出即通知，**几乎零成本**：`_reader` 的 `finally` 本来就在退出时执行，
以前只是自己结束、谁也不告诉）＋ `wait_until_success`（周期重跑直到 exit 0，用于站外条件）。

**三条纪律落地**：只投事实不下结论（有测试钉死投递内容不含"应该/建议你/请修"）；
不许无声挂死（超时也如实通知，轮询下限 5s、时长上限 120 分钟）；
回投**走已有的 `enqueue`** 而不另造通路——它的 worker 启停竞态早已处理过
（"退出与 enqueue 的启动都在 `_worker_lock` 下，无丢任务"），另写一份只会重踩一遍。

**过程中修了自己一处设计问题**：等待器原先把 deadline 捕获成闭包局部变量，导致 `waiters()`
报的剩余时间与实际生效的可能是两个数。改成**以台账里的值为唯一事实来源**。
是写超时测试时暴露的——那条测试本来只想跳过一分钟等待。

**⚠ 一处我差点自欺**：验"模型会不会自己用"时，第一版提示里写了"我不想你反复回来查它好了没"
——**几乎等于把答案说出来**，过了也不能算数。补了中性提问（零暗示）复验才算数。
**这正是 `delegate_implicit` 那条评测任务存在的理由**（"精简 prompt 曾在此翻车"）。

**自检**：新增 `test_offsite_wait` 10 条 + `diag_offsite_realrun` 真模型真跑 **6/6**。
**全回归全绿**（Python 73 文件 + 前端 133）。
**验证状态**：✅ **真跑验证通过**（2026-08-14，定版 v3.70.0）。**本功能无 GUI 依赖**，
不需要 Windows 真机验；但**只在 DeepSeek 上验过**，换模型的选择倾向未知。

**遗留**：W2（等待期间关掉 app → 重开带着答案提醒）**故意不做**——等待期通常几分钟到十几分钟，
这期间关掉整个 app 是低频场景。**先看 W1 被用几次**（ADR 0025 的用量台账能量出来），
半个月没被用过就按 C 档记理由撤掉。

---

## 2026-08-14 — 用量与成本台账（ADR 0025 三期）：把「这次改动是赚是亏」变成可看的数

**起点不是需求，是一次讨论的推论**：用户明确「不训模型、只调用主流大模型」，那么
**模型是外部给定、按 token 付费的输入**，可优化的只有 harness。而没有梯度可以下降，
唯一的优化算法是"改一版、量一量、留下或回退"——于是**人的时间与模型的钱必须并排看**。

**读码体检出 8 处缺陷，全是"静默算错"**（不报错、只给一个自信的错数）。最要命的两处：

- **OpenAI 系的 `cache_read` 恒为 0**。`prompt_tokens` 本就**含**缓存命中部分，旧实现直接
  拿它当"未命中输入"，等于把命中的 token 按全价重算一遍。**真跑实测：一轮 17152 命中 vs
  174 未命中——99% 的输入来自缓存**，按粗估系数算高估 **7.8 倍**。也就是说在高缓存命中的
  用法下，改之前的统计基本没有参考价值。
- **端点不报用量时静默按 0 计**，那一轮成本彻底隐形、看起来免费。现改为估算 + 标记
  `measured=0`，**数要有，但绝不冒充实测**。

**几条贯穿始终的纪律**（都写进 ADR）：只入库事实不入库金额（价格会变，金额落库就无法重算历史）；
币种一等、**多币种分币种汇总绝不相加**；缓存三态分别计价；**没有可信价格就只显 token**。

**中途按用户口径改了两处，其中一处推翻了原方案**：价目字段改成与厂商定价页一致；
成本按**人民币**——随之**删掉了原定的"随包内置牌价"这一档**，因为它全是美元且未核实，
两条叠加就是"安静地给出一个币种和数值都不对的金额"，正是本 ADR 要消灭的东西。

**⚠ 本轮我犯的两个错，都被真机接住**：
1. **面板卡片白底白字**——写了 `var(--panel-bg, #fafafa)`，那变量在本项目**根本不存在**，
   于是浅色 fallback 生效，而本项目**默认暗色**。教训：**别给不存在的变量配浅色 fallback**。
   补救不是"我再看一眼"，而是加 `diag_usage_ui.py` **量对比度**（暗 10.62:1 / 浅 14.36:1）。
   顺带查出 `.table-wrap` 在本项目只定义在 `.bubble` 作用域内，面板里用它根本不会滚动。
2. **`ReferenceError: cost is not defined`**——删函数时漏了另一处引用，真机一发消息就崩，
   而当时 UI 自检 24/24 全绿。**根因是自检只开面板、从没推过 usage 事件**，chip 那条路径
   压根没执行。**没被执行的代码等于没被测**，语法检查也查不出分支里的未定义引用。

**另一条真机反馈值得记**：「填了价格还是提示没填」。计算链路本身是对的，失败模式只有一个——
填的名字与记账用的 `model_id` 对不上。但界面只说"1 个模型没填价格"却不说是哪个，
**用户无从发现**。这是设计问题不是操作问题：现在点名、可一键填入、并从实际有用量的
`model_id` 里选。

**自检**：`test_usage` 31 条 + 前端 133 + `diag_usage_ui` 36 项；**两条 provider 路径各真跑
12/12**（`diag_usage_realrun.py`，anthropic 兼容与 OpenAI 兼容端点各一次）。**全回归全绿**。
**验证状态**：✅ **Windows 真机验证通过**（2026-08-14，定版 v3.69.0）。

**遗留**：订阅制档位的成本不是 token×单价，将来接订阅登录时面板需支持标记「订阅制、只统计
token」；阶梯价/时段折扣不建模（可手填折后均价）；历史数据从落库那天起才有。

---

## 2026-08-14 — 让程序自己解开锁定：从「看懂说明照做」到「点一下是」

**起点**：v3.67.2 的人话弹窗真机验过了，确实弹出中文说明而非 .NET 堆栈。但它**仍然要求用户
去开 PowerShell 粘一条命令**——对不熟命令行的人，这跟没修差得不多。而异常是在 Python 层捕获的，
那时进程还活着，清 `Zone.Identifier` 只需普通用户权限，**程序完全有能力自己做掉**。

**做法**：报错框改成是/否询问；选「是」→ `unblock_tree()` 遍历程序目录删掉每个文件的
`Zone.Identifier` 备用数据流（与 `Unblock-File` 等价）→ 提示重启。

**几个刻意的选择**：
- **问过再动手，默认为否**。这等于替用户抹掉一个安全标记，虽然范围只限本程序目录、
  且用户已经选择运行它了，也不该默认同意；非 Windows / 弹窗失败一律当「否」。
- **不自动重启**。想过点完「是」直接拉起新进程（.NET 加载状态已被污染，必须换进程），
  但那条路我在 Linux 上验不了，**没把握的便利不如确定的一句"请重新启动"**。
- **三种结局分开说**。成功→请重启；部分失败（多为权限）→ 给管理员身份的命令；
  **一个标记都没找到 → 明说"问题可能另有原因"**。含糊的"已处理"会让用户在原地重试到放弃。
- **`walk`/`remove` 做成可注入**，于是"只删 `:Zone.Identifier` 后缀、不碰文件本身"这条
  在 Linux 上就能真测——删错了就是毁产物，这条必须有测试钉住。

**过程中的小插曲**：测试第一版假红，因为我写死了反斜杠期望值，而 `os.path.join` 在 Linux 上
拼出的是正斜杠。**跨平台断言别写死分隔符**——这跟这批 CI 修复的主题恰好是同一类错误。

**首发时被一个假红挡了一轮**（CI run #12）：`test_cwd_absent_or_blank_defaults_to_workspace` 超时。
不是本次改动引入的——它验的是 cwd 回落，却写死 5s 超时，而 **PowerShell 5.1 冷启动在负载高的
runner 上真能超过 5 秒**（`shell.py` 自己的注释里就写着"PS 5.1 的老毛病，实测慢 10 倍"，
只是没人把这条用到测试的超时上）。同一文件前三轮 CI 都过，属时序抖动。
修法照这批 CI 修复一贯的路子——**收敛进底座**而非一处处打补丁：`_shellenv.PLAIN_TIMEOUT`
（Windows 20s / 其余 5s），只用于"让命令有机会跑完"的背景条件；**专门验超时行为的用例保留各自的
显式值**，那些数字是断言的一部分。

**自检**：`test_startup` 4→9，`test_shell` 29/29。**全回归全绿**（Python 71 文件 + 前端 132）。
**验证状态**：✅ **Windows 真机验证通过**（2026-08-14，定版 v3.68.0）。在**没解除锁定**的
v3.68.0 包上走完了整条路：弹窗 → 点「是」→ 提示解除成功 → 重启后正常打开。
自此这个故障对用户是「点一下」，不再是「看懂一段说明并正确执行一条命令」。
