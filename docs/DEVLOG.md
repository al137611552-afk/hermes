...
**状态**：已定版 **v3.62.1** 并推 main+tag。⚠ **未经 Windows 真机验证**（用户要求先推）——
机制是标准 CSS 行为、Chromium 与 WebView2 同源，开发机 Playwright 真渲染验过 62/62。
**待 Windows 复看**（开机顺手，一分钟）：工作区 diff 每行是不是各占一行、`+/-` 行的整行底色是否完整、
反馈框下方没有多余空行。
