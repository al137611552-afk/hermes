...
**✅ 已清（2026-08-25）**：v3.75.0「按停止后联网搜索多久真停」与 v3.75.1「调用 codex 时的
黑框一闪是否消失」两条真机待确认项，用户已在 Windows 真机验证通过；CHANGELOG / DEVLOG /
本文件三处均已回写（这两版走的正是上面警告的"先定版推送、验证后补"那条路，这次没有漏回写）。
留作参照：若今后仍见零星闪，先按频次分辨归属——hermes 这批是调用开始/结束各一下、次数固定，
codex 自己在沙箱里执行命令起的 shell 是跑到一半每条命令闪一下（它的孩子，hermes 够不着）。
...
- **B3 hooks 事件面扩容** —— **✅ 前两个已做并定版 v3.76.0（2026-08-25）**：`UserPromptSubmit` 与 `Stop`
  已实现并入库（详见 DEVLOG 同日条目；crazy 自驱轮刻意不触发）。**只剩 `PreCompact`。**
  原始描述如下：现只有 `PreToolUse`/`PostToolUse`。最值钱的三个缺口：
...
8. **对标主流 B 档**（B1 已降 C 档不做，见上方「对标主流 agent」）：
   B2 外部技能 sanitize → ~~B3 hooks 事件扩容~~ **B3 已做掉三分之二（v3.76.0，2026-08-25）**：
   `UserPromptSubmit`/`Stop` 已定版，只剩 `PreCompact`（已降优先级，见 DEVLOG 同日「遗留」里的投入判据）。
