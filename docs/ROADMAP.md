...
> **现状：v3.66.0（2026-08-13，已定版推 main+tag）**。本文件分三部分：
...
- **v3.66.0 分批评审的 UI 层待 Windows 验**：`reviewer_done` 新增 `batch`/`batches` 字段，
  分屏讨论一轮里会出现 `2 × 批次` 段进言，需真机看排版别挤成一团。**引擎已真模型验过 7/7**
  （`scripts/diag_review_batch_rescue.py`），只差这一层。验完记得回写 CHANGELOG/DEVLOG/PRD 三处。
