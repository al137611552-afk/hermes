...
**验收**：✅ transient 自动重试至成功 / 撞上限返回最后失败 / 非 transient 不误重试 均有测试。全回归绿（Python 48 + 前端 30）。**首个改运行时行为的块——建议 Windows 真机验一次瞬时失败恢复。**
...
- **块 B：已完成**（2026-06-30，事实层标准化；前端摘要条待 Windows 视觉验）。
- **块 C：已完成**（2026-06-30，9 类错误分类 + ADR 0015）。
- **块 D：已完成**（2026-06-30，瞬时 IO 自动重试；首个行为变更，待 Windows 验）。
- **块 E：下一步**——World State + Failure Memory。
