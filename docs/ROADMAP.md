...
**新增待确认（2026-08-24，v3.75.1）**：Windows 上调用 codex 时的"黑框一闪"是否消失。
本机只能验到"标志真的传下去了"（假 Windows 真调用 7/7），**看不见窗口**。
注意可能仍有一批闪不归我们管——codex 自己在沙箱里执行命令起的 shell 子进程；
可按频次分辨：hermes 这批是调用开始/结束各一下、次数固定，codex 那批是跑到一半每条命令闪一下。
