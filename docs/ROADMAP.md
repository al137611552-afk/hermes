...
- A1 定义 `Need` 枚举（9 个）+ `Evaluation` dataclass（metrics/signals/issues/confidence）。放 `src/agentcore/agent/contract.py`（纯逻辑，可单测）。
- A2 把 crazy 现有 verdict（`[[DONE]]/[[CONTINUE]]/[[NEED_USER]]/[[PHASE_DONE]]`）映射到 Need，**行为不变**。
- A3 重构 `loop.py` 每个 nudge 触发点为"读 Need → 选注入"，注入内容保持原样。
- A4 全量回归（Python + 前端）必须绿 = 等价重构证明。

**交付物**：`contract.py` + 单测；`conversation.py`/`loop.py` 走新契约但行为零变化。
**验收**：回归全绿；crazy 块2/3/4 行为与块A前逐字节一致。
**为何先做**：所有后续能力都挂在这个契约上；先证明它不破坏现状，风险最低。
