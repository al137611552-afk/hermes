...
- 状态：**已接受**（T1 源码自更新已实现并定版 v3.52.0；**更新提醒自 v3.53.0 起默认关闭**——
  用户不要这个提醒，开关 `agent.update_check`，设 `true` 即恢复。能力本身保留）
- 日期：2026-07-03
...
**T1 对 exe 用户等于没有**：非 git 仓库时提示"请用下载页更新"，而下载页给的是**源码**——
exe 用户拿到源码也用不了，**还得自己在 Windows 上跑一遍 `build.ps1` 重新打包**。
即 T1 只服务"源码装"的自用场景，一旦以 exe 形态用/分发，更新链路是断的。

**根因不在更新器，在发布流程**：仓库**没有 CI**，exe 只能手工构建
（`git pull` → `build.ps1` → 压缩 → 上传），所以"下载页"上不一定有对应新版的 exe。
先补这一环（tag 推送时由 CI 在 windows runner 上产出 exe 并挂到 Release），
"下载页更新"才真的成立；在那之前谈 T2 的自动换文件更新是空中楼阁。
