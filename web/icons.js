// icons.js —— 全站图标的**唯一来源**：官方 lucide 矢量（ISC，v1.22），逐字复制，非手绘近似。
// 设计稿（Figma Make 导出）用的就是 lucide 图标，这里用同一套原始矢量做到 1:1，并统一
// viewBox=24 / stroke-width=2 / 圆角端点。用法：icon("plus", 16) → 完整 <svg> 字符串。
// UMD：浏览器里挂全局（app.js 直接用 icon()/LUCIDE），Node 里导出供单测。
(function (root, factory) {
  const api = factory();
  if (typeof module !== "undefined" && module.exports) module.exports = api;
  else { root.LUCIDE = api.LUCIDE; root.icon = api.icon; }
})(typeof self !== "undefined" ? self : this, function () {
  // name -> 内部矢量元素（原样取自 lucide-static，仅去掉外层 <svg>）
  const LUCIDE = {
  ...
  // 统一属性；size 只设 width/height（矢量按 24 网格缩放，线宽随之等比，视觉一致）
