...
**版本号已升至 3.52.0**（技能能力为新增功能，按 SemVer 走 minor）。条目暂留在 Unreleased：按项目规矩「验证通过才定版」，本段与其上的步数上限/前台流输出一样待 Windows 真机验。
