...
**已实现待 Windows 验**：**研究检索·治本 option B**（`web_search` 宽召回 count=30 + 确定性重排 `rerank_results`：查询词覆盖度排序 + 单域封顶 2 控源多样 + 去重，治"直吞 Bing 前几条噪声/单站霸屏"，无 key、无模型、无延迟；test_web +4）——待真网搜「苹果 水果/2026 显卡价格」观察后随下一版定版。
