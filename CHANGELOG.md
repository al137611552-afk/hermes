...
- **浏览器穿透：没装 Chrome 时有头模式弹不出浏览器，且没人说得清为什么**（2026-08-26 真机）。
  用户开了穿透但没勾有头，遇登录墙时点「切到有头并打开这页」——浏览器没弹出来，
  只能自己补一句"浏览器未弹出"再点「我做完了」；装好 Chrome 后重试仍失败，模型建议重启，
  重启后**整轮检索成果全丢**。四处一起修：
  - **没有 Chrome 自动回退 Edge**（新 `browsers.py`）。`browser_mcp_args()` 此前写死
    `--browser chrome`，而 `msedge` 本就是 Playwright 合法 channel、**Windows 一定有 Edge**——
    这件事根本不必让用户去装。顺带修了个隐性误判：**用户级安装**（无管理员权限，装在
    `LOCALAPPDATA`）的 Chrome 以前会被当成"没装"。
  - **切换前先探测**：`handoff_open_target()` 先看本机有没有浏览器（毫秒级、只看文件在不在、
    不起进程），一个都没有就当场拒绝并给出路，而不是切完有头、弹不出窗口、让人对着不存在的
    浏览器点「我做完了」。
  - **失败进模型上下文**（`Conversation.notify_system`，复用后台进程回投那条已验证的 enqueue 通路）。
    此前 `nav.run()` 失败只回前端弹个 toast，**模型完全不知道浏览器没起来**，于是按"人已经去登录了"
    继续走，最后猜出代价最大的出路。现在明确告诉它：别当用户已登录成功、
    **别建议重启——切有头只重启 MCP server，不需要重启应用**。
  - **前端失败信息常驻**：不再是几秒就没的 toast，多行留在换手条上、按钮变「重试」。
- **回合内边跑边落库：应用中途退出不再让这一轮的搜索成果全部蒸发**
  （`AgentLoop` 的 `on_message` + `_LiveList`）。此前一轮里的消息是**跑完才一次性写库**的，
  于是崩溃/强关/断电＝库里一条都没有，重开会话只看得到上一轮。按「停止」不受影响
  （那条路 loop 正常返回、照常落库），丢的只有**非正常退出**。现在一条消息一次 SQLite 写入，
  退出时最多只丢正在进行的那一步。
  **对照过主流实现**：Claude Code 的 `~/.claude/projects/*.jsonl` 与 Codex 的
  `rollout-*.jsonl` 都是**一条一行、发生即写**（实测 tool_use 与它的结果相隔 1.2s / 84ms
  各写各的行），没有"回合结束再统一落盘"这一说。
  - 顺带修好了换手落盘：`_snapshot_handoff` 读 `self.history`，而本轮的工具调用以前要等
    回合结束才进 history——它记下来的其实是**上一轮**的现场。
  - **配套**：`repair_interrupted_tail`（`context.py`）。断在"assistant 发了 tool_use、
    tool_result 还没落库"时，历史直接喂回模型会被 API 打回（tool_use 没有配对的 tool_result），
    那就从"丢内容"变成"这会话再也发不出消息"。补一条注明中断的 tool_result，
    **不删那条 assistant**——它是"当时打算干什么"的唯一线索。Claude Code 的存档里对应的是
    `[Request interrupted by user for tool use]`，同样是补而不是删。
- **子 Agent 的检索过程也落库了**（`messages.sidechain` 列 + `run_subagent` 接 `on_message`）。
  此前 `run_subagent` 只把最终摘要回灌主 Agent，中间过程既不进 history 也不进库——
  而委派型调研里搜索量最大的恰恰是这部分，中途退出＝子 Agent 白搜。
  **进同一张表、打标记区分**（对齐 Claude Code 存档里的 `isSidechain`）：
  - `get_messages()` **默认不返回**旁链——恢复会话时它们要是混进主历史，就等于把子 Agent 的
    中间过程塞回主模型上下文，委派"独立上下文、只回摘要"的意义当场没了；
    要看全貌传 `include_sidechain=True`。
  - `recall_history` 搜得到它们——**落库的意义就在这**：模型能把子 Agent 已经搜过的捞回来，
    不必重复搜一遍。
  - `truncate_messages_after` 改为按**主链**计数（旁链与主链交错躺在同一张表里，
    直接按行号切会砍错位置），并把被丢弃那几轮的旁链一并带走、不留孤儿。
  - 旧库自动补列，历史照常打得开。

### 移除

- **「继续上次的会话」提示条**（2026-08-27 用户判定，整块删除：后端判定、前端提示条、
  样式、配置项 `agent.resume_hint_minutes`）。它只记得住"上次在跑哪个任务"，多条任务在跑时
  还只记得住一条，而会话本来就带着任务清单和笔记——再提醒一次没有增量。
  恢复的价值在**进行中已搜到的内容不丢**，那条已由上面的边跑边落库解决。
  Claude Code 与 Codex 都没有这种提示条，恢复是用户显式发起的（`--resume` / `codex resume`）。
- **重连 MCP 后，正在跑的那一轮不再一路「未连接」到底**（`McpManager.succeed_to`）。
  2026-08-26 真机：换手中途点了「切到有头」，之后每一次 `browser_*` 都报
  `MCP server 'browser' 未连接`，`web_fetch` 的浏览器兜底同样失效，模型据此判定"浏览器服务故障"——
  而设置面板显示连接正常，**两边说的都是实话**。重连是"close 老 manager + 建新的"，
  但**正在跑的那一轮握着回合开始时快照的 registry**，里面的 `McpTool` 把 caller 绑死在老 manager 上，
  于是撞的是它已经清空的 `_sessions`。现在老 manager 交班给新的、调用照常转过去（owner 一并透传，
  「停止」仍认得出人）；没有接班人时行为不变（照旧报未连接）。
- **「继续上次的会话」不再在用户自己新建会话时冒出来**（`Api.list_sessions`）。
  判定条件只看得见"当前落在空白会话"，分不出这是重启后被动落进来的、还是用户点了「新建会话」——
  于是跑着跑着新建一个，就被提示"继续上次的会话"，而上次那个正是刚才那个（2026-08-27 用户反馈）。
  改成**一次启动只判一次**：闸放在调用方而不是纯函数里，"进程是否刚启动"是状态，留在外面才好单测。
- **换手时自动把已拿到的东西落盘**（`Conversation._snapshot_handoff`）：写进工作区
  `.hermes/handoff/<时间戳>-<目标>.md`，含换手目标、验证方式与**本轮实际调用的工具及结果摘要**。
  换手是最容易"人去操作 → 中途出岔子 → 最后重启"的节点，而换手请求本身只活在内存里、重启即蒸发。
  **落工作区文件而不是 session_notes**——notes 要回到同一个会话才看得见，
  而这条路上最常见的恰恰是重启后落在新会话里。**靠模型自觉写 notes 不够**：
  它想不起来的时候正是最该记的时候，所以由代码做。
- **「继续上次的会话」提示条**（`agent.resume_hint_minutes`，默认 60 分钟，0=关）：
  启动时若落在空白新会话、而上一个会话在阈值内还有活动，就在会话栏顶部给一条可点的提示。
  **只提示不自动切**——自动切会让"我就是想开个新的"每次都要先关掉一个旧的。
  治的是"重启后成果像是全丢了"（其实还在库里，缺的只是一条指路）。
- **模型 API 的 402 不再被误读成搜索配额用尽**（`providers/base.py` 的 `account_problem`）。
  真机踩到：中转端点回 402 `insufficient_credits`，界面只打一行
  `APIStatusError: Error code: 402 - {...}`，用户据此以为是 Firecrawl 没额度了——
  而那条路**根本不经过这里**（`web.py` 的 402 走 `FirecrawlQuotaError` 自动降级到免 key 链路）。
  现在会说明这是**模型 API** 的鉴权/计费/权限问题、**指名是哪个模型档与端点**，并提示
  委派子任务用的可能是 `agent.subagent_model` 指定的**另一个账户**。
  - **账户级错误永不重试**（重试多少次都是同一个答案），且与 429 严格分开：
    **状态码优先**——429 限流的文案里也常带 "quota"，据此判死付费链路就会把该退避的当成没钱。
- **子任务死在模型侧时不再被当成"做完了"**。provider 把流式失败转成 `error` 事件后 loop 正常返回
  （**不抛异常**），委派侧原先据此把残缺结果当成功摘要交给主 Agent——主模型据此下结论，
  错得毫无察觉。现在认领 `loop.stream_error` 并明确标注「子任务因模型调用失败未完成」。

### 新增

- **`/诊断` 命令与「这次到底用的哪个模型档」自查报告**（`modeldiag.py` + `Api.get_diagnostics`）：
  把主对话 / 委派子任务 / 压缩摘要各自实际用的档名、provider、端点、key 取自哪个环境变量
  （**只说来源与设没设上，绝不打印 key 本身**——这份报告就是拿来贴出去求助的）一次摊开。
  起因是排查 402 时绕了几轮：模型侧报错**不带档名**，而一台机器上通常好几个档同时在工作。

### 变更

- **流式渲染改成按时间片、不按 token**（`web/pure.js` 的 `createStreamPacer` + 新
  `bridge/coalesce.py` 的 `ChunkCoalescer`）。病根是 O(n²)：每来一个 token 就把整条消息
  重新 `marked.parse` + 全量重跑代码高亮，并发几个会话时这些渲染全挤在同一个 UI 线程上。
  - 前端：增量先攒 80ms 再渲染一次（**不用 rAF**——16ms 比模型出字还快，一帧一 token 等于没节流）；
    **流式不高亮、定稿才完整渲染**。首片立刻出，开头不会显得没反应。
  - 后端：`evaluate_js` 按对话攒 80ms / 8KB 再发一次。**顺序是硬约束**——攒着的文本必须在同一
    对话的任何其它事件之前吐出去，否则界面上工具块会跑到那段话上面。
  - 真实流式数据实测：一条 875 字的回复，`evaluate_js` 从 **505 次降到 66 次（1/7.7）**。

### 回退说明（同日「委派并行优化」）

同一天基于三轮 A/B 真跑做过四条委派/检索优化，**真机使用后判定质量下降，本版整块回退**：

- **提示词改动已逐字回退到 v3.76.1**（`delegate` 工具说明、researcher 角色指令、
  `SUBAGENT_DIRECTIVE`、`config.yaml` 系统提示词）。**根本错误**：给模型加了一条
  「先估自己做要几次检索、少于 5 次就别拆」的**自我估算门槛**——而模型估不准，
  调研类任务开搜之前根本不知道要挖多深，于是把本该多角度并行调研的任务当小活直接开干，
  连带也不再加载调研技能。**更讽刺的是，用户举的那道装机配置调研，正是三轮数据里
  委派赢的那一道**（串行 1200s 没收完、并行 616s 给出完整答案）——我却拿另一道小题
  （uv vs pip，2~3 次检索）得出的阈值当了通则。
  同批的 researcher「拿到 2~3 个来源就停止检索」也与该角色原有的「逐层下钻、别只给一级摘要」
  直接打架，会让子 Agent 变浅。**教训：别把一道题的实测阈值写成需要模型自我估算的通则。**
- **同回合共享检索缓存**（`webcache.py`）：**默认关**（`web.turn_cache: false`）。
  三轮真跑命中 0 次，它治的"子 Agent 重复搜同一片领域"经查询词分析证伪并不存在。
  代码与 14 条测试保留，需要时打开即用。
- **子任务墙钟上限**（`agent.subagent_timeout_s`）：**默认 0=不限**。三轮真跑一次都没触发，
  它是护栏不是修复。
- 关掉这两个开关后，**搜索与委派路径的行为与 v3.76.1 逐字节一致**。
- 诊断脚本 `scripts/diag_delegate_speed_realrun.py` 保留（新增 `--cap`、缓存命中账、
  **查询词与近似重复分析**——正是最后这项把当初的推断证伪的）。

> 自检：Python 全回归 **93/93 文件 0 失败**、前端 **153/153**。
> **Windows 真机待验**：浏览器路径探测与 Edge 回退是平台相关的，Linux 上只能验纯逻辑。

## [3.76.1] - 2026-08-25

**修 v3.76.0 引入的 CI 回放门红**。发版当轮 `release` 绿、`ci` 红，红在块 V3 的离线回放：
`bugfix` 报「第 1 次模型请求没有对应录音」。

### 修复

- **改工具说明＝改请求指纹**。v3.76.0 给 `read_file` 的 description 加了一句用法提示，
  而**工具 schema 是请求的一部分、进 cassette 指纹**——一个字都不能白改，整套评测录音当场失效。
  病根不在代码逻辑（三层回放门在还原描述后立刻 19/19 全绿），在"哪些东西算请求"。
  **提示挪进结果尾巴**（只在真读了一半时出现）：① 时机更对——不必每轮都占 schema 篇幅，
  正需要时才说；② 不动指纹，录音继续有效。理由连同这次教训一起写进了 `fs.py` 的注释。

> 验证：三层回放门 L1 1/1、L2 12/12、L3 6/6；Golden 67/67；Python 全回归 88 个文件 0 失败。

## [3.76.0] - 2026-08-25

**两件小而确定的事**：hooks 事件面补上前两个缺口；`read_file` 不再让模型瞎猜文件有多大。
纯后端，无 GUI 变更。

### 新增

- **hooks 两个新事件（对标 B 档 B3 的前三分之二）**：
  - **`UserPromptSubmit`**（用户消息进内核前）：退出码 **2=拦截本轮**——不落库、不建会话、
    不调模型、不触网，stdout 作为拒绝理由告诉用户；**0 且 stdout 非空=作为附加上下文注入本轮**
    （用途：每轮自动注入项目上下文）。挂在 `save_images` **之前**——被拦的那一轮不该在工作区
    留下副作用。
  - **`Stop`**（一轮终态）：payload 带 `reason ∈ {done,error,stopped}`，**stdout 不回灌模型**，
    只作通知/副作用（收尾跑测试、发通知）。
  - 两个事件都没有工具名，`matcher` 空=匹配全部；写了 matcher 的 hook 自然落空
    （等于给这类 hook 一个显式的"不参与"写法，不必另开一套语义）。
  - **crazy 自驱轮刻意不触发**：一个自驱任务动辄十几二十轮，每轮响一次会把「Stop 时自动跑测试」
    变成跑二十遍。「重新生成/编辑重发」同样不触发——那不是新的用户输入。
    **代价说在明处**：整个 crazy 任务结束时也没有 Stop，任务级收尾是另一件事。
- **`read_file` 没读完时报「本文件共几行」**：模型据此判断是接着读完，还是文件太大该改用
  `grep_search`/`code_outline` 定位——这句用法提示跟数字写在一起，**光给数字不给用法，
  模型多半照旧一段段翻**。（v3.76.0 原本把它写进工具说明，随即被 CI 回放门逮住，
  v3.76.1 挪进了结果尾巴，见下。）读到末尾则不报（那时最后一行的行号就是总数，再说一遍是纯噪音）。
  数行复用同一个已打开的句柄往下数，不重读文件；封顶 20 万行，超了报 `200000+`。

### 修复

- **`_run_turn` 的第四个终态出口漏挂 Stop**：`build_provider` 失败（模型档/密钥配错）那条
  同样是一轮的终态，却没有收尾。四条出口各写一遍的写法，**漏一个不会有任何测试发现**——
  现在 `hook_runner` 提到 provider 构造之前建好（建晚了那条出口没得用），四条统一走 `_fire_stop()`。

> 验证：Python 全回归 **88 个测试文件 0 失败**、前端 **145/145**；新增测试 15 条
> （hooks 12 / read_file 2 / 第四出口 1）。**活性验过**：去掉 crazy 例外那半行，
> `test_stop_hook_not_fired_in_crazy_round` 当场变红。另有一支只走公共面
> （`config._norm_hook` + `Conversation.send_message`）的中立探针 8/8，
> 该探针对未改动的 v3.75.1 只给 3/8。
> 本版**无 GUI 改动**，Windows 侧由 CI 的 windows job 覆盖全回归。

## [3.75.1] - 2026-08-24

**不再闪黑框**。用户真机反馈：调用 codex 干活时经常弹出一个终端窗口然后秒关。
✅ Windows 真机验证通过（2026-08-25）。
...
> 前端 145/145）。✅ **Windows 真机验证通过（2026-08-25）**——黑框不再闪。
...
而**另一个**会话 B 正在跑的 codex 反倒被停掉了——同一个按钮，该停的没停、不该停的停了。
两半病根不同，一并修掉。
✅ Windows 真机验证通过（2026-08-25）。

### 修复

- **停止会停到别的对话头上**（2026-08-24 真机）。`McpManager` 是**跨对话共享的单例**，
  在飞的工具调用却记在一个全局集合里、`cancel_all()` 无差别取消所有 future。
  于是"停 A"＝取消**整个进程**的在飞 MCP 调用，B 正在跑的 codex 当场被打死；
  而 A 跑的是 `web_search`（根本不走 MCP），那一下对 A 一点用没有。
  现在在飞调用**按 owner（对话 cid）记账**，`cancel_all(owner)` 只取消该对话的
  **外加无主的**（没绑 owner 的调用没有别的出口，宁可多停也不能留下停不掉的活）；
  不传 owner 仍是全停，留给关停/退出。子 Agent 的调用算在发起它的对话名下——停本对话要连它一起停。
- **`scripts/mcp_echo_server.py` 在 mcp 2.x 上起不来**：2.0 把 `mcp.server.fastmcp.FastMCP`
  改名成了 `mcp.server.mcpserver.MCPServer`，子进程 `ModuleNotFoundError`，
  而客户端只看到一句 "Connection closed"。两个版本都认（同 `tool_input_schema` 的既有纪律）。

### 新增

- **阻塞型检索工具的停止令牌**：`web_search` / `web_fetch` 接收本对话的 `_cancel`，
  在**每一次"换个源再试"之前**查一次。此前一次搜索最坏要串上
  firecrawl → bing rss → bing html → ddg → 读 3 条正文，**每跳一个 timeout（默认 20s）、
  中间一个检查点都没有**——按了停止只能干等好几个 20s 串完。
  立场是**不打断已发出的请求**（urllib 没有取消接口，硬断会漏连接），只掐掉后面的回退；
  最坏等待因此压到"当前这一跳"以内。两条配套规则：
  - **有货就回货、没货才报停**：停止落在拿到结果之后，已经搜到的（尤其是花过 credit 的）
    照常给出去，只是不再读正文，并在结果里说明「已停止，未再读取正文」；
  - **说清是"没试"而不是"试了没成"**：web_fetch 的 Firecrawl/浏览器兜底被停止跳过时，
    错误里明写未再尝试——否则排查会去查一条根本没跑过的路。
- **`scripts/diag_stop_realrun.py`**：不 mock 任何一层的真跑验证（**真起 stdio MCP server
  并发两个 owner、真出网搜索**）。单测只能证明"检查点被调用了"，证明不了"按下去真的马上停"。
  本机 16/16 通过。配套给 `mcp_echo_server.py` 加了个 `sleep` 慢工具——没有慢调用就复现不出归属问题。

> 验证：本机真跑 16/16（MCP 归属取消 8 项、web_search 停止令牌 5 项、web_fetch 阶梯 3 项）+
> 全回归绿（Python 87 个测试文件 0 失败、前端 145/145）。改动全在后端，
> ✅ **Windows 真机验证通过（2026-08-25）**：界面上按停止后，联网搜索确实停得下来。

## [3.74.0] - 2026-08-21

「配得上、也说得清」的一版：**联网检索的两半配置（key 与档位）收进同一屏**，并把"现在实际走的是哪条链路"当作面板的第一职责说破。
✅ Windows 真机验证通过（2026-08-21，A–E 全清单）。

### 新增

- **设置面板新增「🔍 联网检索」页**：Firecrawl 的 **API Key 与档位放在同一屏**，
  填完即写回 `<安装目录>/.env` 并即时生效（不用重启、不用手编任何文件）。
  起因是一次真实的排查困境（2026-08-21）：key 只从环境读、档位在 `config.yaml`，
  **两个都对才生效**，而任一边缺了都只表现为"搜索照旧走 bing"——
  产品里没有任何一处说得出缺的是哪一半。所以这一页除了能填，还负责**说破**：
  - 面板与左栏徽标报的是**实际生效档位**而非配置写的那个：没 key 时显示「缺 key」，
    配额用尽时显示「配额用尽」，都直说"现在实际走的是免 key 的 Bing + DDG"；
  - 换 key 时顺带清掉「配额用尽」的进程内粘滞标记——"配额用尽换把 key"
    正是会来这一页的主要原因之一，不清则新 key 当场又被当成已用尽；
  - Key 只回掩码预览、绝不回明文（延续设置面板既有立场，有自检钉住）。
- **枚举型设置的统一机制** `CHOICES_SPEC` / `choices.json`（与既有「限额与预算」的
  `LIMITS_SPEC` 同构，只是值域是有限候选而非数字区间）：单一数据源同时驱动
  ①校验/持久化 ②前端渲染下拉。既有的 feature_flags 只覆盖 `agent` 段且值是布尔、
  limits 只吃数字，"三四个具名档"两边都放不进，硬塞会把两个机制的语义都搞浑。
  脏值与白名单外的 key 一律跳过，绝不落盘。

### 修复

- **`.env.example` 与打包模板（`pack.py` 的 `ENV_TEMPLATE`）里没提 `FIRECRAWL_API_KEY`**，
  只列了几个模型 provider——于是"这把 key 到底配在哪"在产品里根本查不到出处。
  两处都补上注释行，并标明它是**检索源、不是模型 provider**，没有也能用。
