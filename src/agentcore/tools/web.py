...
#
# **默认不是 always**。原先的理由是配额算术（search=2 credits、免费档 1000/月，
# 一个研究型任务搜 7–8 次 ≈ 一个月只够 60–70 个任务）。
# 2026-08-21 拿 6 个真网任务跑了一轮 primary vs always 的 A/B，结论换了个方向：
#   - **通过率一样**：6/6 vs 6/6
#   - **配额也几乎一样**：39 vs 37 credits（用 team/credit-usage 前后差值实测，差值在噪声里）
#   - **但 always 每个任务都更慢**：6/6 全部变慢，+7.6s ~ +109s，合计 +298s ≈ **+52%**
# 也就是说 always 花掉的不是配额、是**时间**——每次读页都多绕一趟 Firecrawl 的渲染，
# 而这些页免 key 链路本来就读得动。（n=1、真网每跑一变，但"6 个全变慢"是方向一致的信号，
# 不是随机噪声；重跑命令见 docs/DEVLOG.md。）
#
