...
- **Firecrawl（FR-11.1d）的适用面比想象窄，别当银弹**：2026-08-20 真网实测——`search`=**2 credits**、
  ...
  **别开 `always`**：2026-08-21 的 A/B（6 个真网任务）实测——通过率一样（6/6 vs 6/6）、
  配额也几乎一样（39 vs 37 credits），但 **6 个任务无一例外全都变慢**，合计 **+52%**。
  它花掉的不是配额、是**时间**（每次读页多绕一趟渲染，而那些页免 key 链路本来就读得动）。
  批量评测里另有一条配额账：一个研究型任务搜 7~8 次，一个月只够 60~70 个任务。
